...
percentage tolerance AND a rupee one or per-bill rounding raises false alarms. S661 adds the module's
colour vocabulary (`src/modules/pos/posSignals.js`): the floor tile's strip answers *does this table
need me?* rather than repeating the occupancy a waiter can already see, the KDS strip carries
lateness rather than the stage its own column already names, and a close type, a rank and a delivery
partner are categories that take brass or grey — never a signal colour. Reach for that file before
picking a colour anywhere in POS.
