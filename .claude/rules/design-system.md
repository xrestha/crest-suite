...
paths:
  - "src/**/*.css"
  - "src/components/**"
  # S770: `src/pages/**` and `src/modules/**` dropped (117k loaded on every page and module file).
  # The page-level rules a JSX-only edit needs are restated in page-layout.md, which loads there.
  - "DESIGN.md"
---

Rules summarised in page-layout.md: if you change one, update page-layout.md in the same change.
...
**A tint derives from the TOKEN, never from one preset's hex (S683 reversed the older convention below).** Write `color-mix(in srgb, var(--theme-red) 12%, transparent)` — or `colorTint(token, pct)` from `pricingPlans.js` — so the tint re-tones with the preset. The old convention was a literal `rgba(248,113,113,0.12)`, which the hook accepts because it resolves back to the Dark palette; that is exactly the problem: **334 such tints were the Dark preset's red, brass, green, amber and violet frozen under every light theme** (Light's red is `220,38,38`, not `248,113,113`), and the hook could not see it because a Dark hex IS a documented colour. Dark declares no `*Text` keys, so base and variant are byte-identical there and nothing looks wrong in the preset everyone develops in — the same structural fact behind the `*-text` rule. All 334 were converted by a scripted sweep (`.claude/rules/design-system.md` is where the tell is recorded: `rgba(` with one of the five Dark triplets in a `.js`/`.jsx`/`.css`). An rgba literal survives in exactly four places, each because `var()` cannot resolve there: `:root`'s own `--theme-focus-ring` token, a `var(--x, rgba(…))` fallback in the boundary/loading screens, an SVG presentation attribute, and a print HTML string. Anywhere else it is drift. The older rule's two named traps still stand, and were fixed in S424, both of which shipped as real bugs and were fixed in S424: **a solid signal-color fill needs a paired foreground token, and only `--theme-accent` has one** (`--theme-accent-text`). `--theme-red` ranges from light (`#f87171` Dark) to dark (`#dc2626` Bright) across the ten presets, so no single foreground contrasts on all of them — use DESIGN.md's tint pattern (alpha fill + full-opacity signal text) for red/amber/green instead of a solid fill. And **`rgba(255,255,255,…)` as a "slightly brighter border" is invisible on the five light presets** — step the border to the accent at low alpha instead.
...
- `stat-card--compact` — the dense tier of that tile, for a strip with a REAL gap: `8px 16px`, no seam pull, shadow kept. **Added S700 because "hand-roll only the property that genuinely differs" had produced four different KPI cards.** ClientDashboard, OwnerDashboard and AdminDashboardOverview each typed `.stat-card`'s five box properties out inline and then landed on their own padding — `10px 14px`, `14px 16px`, `12px 14px` against the class's `20px` — so none of them could receive `tabular-nums` (S594) or `line-height` (S657), and two of them sat in a `gap: 0` `.stat-grid` **without** the -1px seam pull or the shadow reset, butting 1px borders into 2px double rules with the shadows overlapping at every join. A density that three files want is a variant, not three copies
- **A hand-rolled copy of one of these keeps only the rules the class had on the copy date.** Separate the decisions: take the class for everything it still describes, hand-roll only the property that genuinely differs — and when the differing property turns up in a third file, give it a name. **The tell that a copy has gone stale is a value on it that exists nowhere else in the product**
- `dash-section` (28px) / `dash-row` (16px), both self-zeroing on `:last-child`, stepping to 16/8 under 768px — the dashboards' vertical rhythm (S700). Declared after `.stat-grid`, so a grid that IS a section takes `stat-grid dash-section` and wins the tie. The `:last-child` is what makes them safe to apply unconditionally: dashboard JSX is conditional almost everywhere, so which block renders last depends on which modules a client bought, and a plain bottom margin would stack on `.main-content`'s own 32px padding for some clients and not others
...
- `choice-chip` / `choice-chip__name` / `choice-chip__meta` — an option chip in a `radiogroup` or checkbox group (the till choice window, S759). Carries `[aria-checked="true"]` (accent border + inset ring, so the box does not grow), `:disabled`, and the focus pair; pair it with `moveRovingFocus` from `src/shared/rovingFocus.js` so the group is one Tab stop. `kind-card` is the same idea for the three "what is it?" cards in the option-group dialog. The guest menu keeps its own `.gm-option` in `guestMenu.css` — the same states on the app's tokens since S767, with a square mark (filled for a checkbox, an inner square for a radio) because the guest sheet's layout differs, not its palette. `gm-steps` / `gm-steps-bar` / `gm-review` / `gm-review-row` / `gm-review-group` (S760) are the guest sheet's step-by-step mode for a build-your-own dish — size step first, a `role="progressbar"` bar, a Review list before Add — and live in that same file
- `.btn[aria-disabled="true"]` — a button that must stay PRESSABLE so the press can say what is short (the choice window's Add, the guest sheet's Add): opacity 0.6, deliberately lighter than `:disabled`'s 0.55 so it does not read as dead. **The guest sheet overrides it to 1 (S767)**: on the accent fill 0.6 measured 3.36:1, and that button's label already says what is missing ("Choose a size")
...
- `arrival-alert` / `arrival-alert--urgent` / `arrival-alert__icon|body|title|detail|action|mute` — the loud app-wide banner `ArrivalAlert` renders (S763): something arrived that a person has to act on and nobody has. Fixed at the top, z-index 3000 (above the POS till layers at 1000 and the Calculator at 2500, below `Tip`'s 9999), `role="alert"`, and deliberately **not** a scrim or a focus trap — see the scrim rule below. It publishes its own measured height as `--arrival-alert-h` when asked to, which `.layout-root` pads by; never hardcode that reservation, the banner wraps under 600px. Reach for the component, not the classes
...
- `till-hit` / `till-hit--row` / `till-tile` / `action-error--till` — the POS order screen's own hooks (S776). The screen is inline-styled, so these carry what an inline style cannot: the `pointer: coarse` 44px floor (`till-hit` both dimensions, `till-hit--row` height only, `.ticket-btn` joins the row rule) and the product's focus pair (`till-tile` is focus-only, for the menu tiles). `action-error--till` is `.action-error` at a 10px bottom margin, for the message line above Send. Measured S776: 0 of 32 order-screen controls under 44px at 820 touch. `input::placeholder` / `textarea::placeholder` take `--theme-text3` app-wide at the same time, so an inline-styled field no longer shows the browser's grey
- `month-status` / `month-status__head|steps|step|label|state|mark|link` — the payroll-month strip `PayrollMonthStatus` renders (S768). `month-status__mark--done|open|none` is the ONLY colour on it (green-text ✓, amber-text △, fog —) and the sentence beside the mark always says the same thing in words. Wear it with `.card`; it is a summary card, not a banner
- `bulk-approve` / `bulk-approve__text` — the batch-approve strip at the head of an HR queue's card (`BulkApproveBar`, S768). States the count and the total before the press; its button is a neutral `btn btn-ghost btn-sm`, like the per-row `DecisionButtons`, because approving is routine work and the outcome shows in the status badge afterwards, not in a button's colour
...
### Navigation and design-hook rules (moved)

Nav item visibility on the item (S638/S639), the "you are here" longest-prefix match (S763) and one icon per route (S606): `.claude/rules/navigation.md` (auto-loads for `Layout.js`, `Layout.css` and `CommandPalette.js`). The design-hook ignore-path rule (S763) is in `.claude/rules/design-tokens.md`.

### Two decisions settled in S613, so they stop being re-litigated

**The phone IS a supported reporting surface.** `.stat-grid` is
`repeat(auto-fit, minmax(200px, 1fr))` and reflows on its own; **25 report pages had overridden it
...
its cards hold no interactive children. Table Management's grid (POS Setup since S776) *looks identical*, had a bare
...
on its "wants to subscribe" dot anyway. (Third instance avoided rather than found: `ArrivalAlert`'s
entrance, pulse and bell-ring all went into `ArrivalAlert.css` from the start, S763 — and its
reduced-motion branch keeps the urgent fill at the LOUD end rather than reverting to the quiet one,
because switching off the pulse must not also switch off the verdict.) It is `.pulse-dot` now. Moving it surfaced the second
...
## Theme tokens and presets (moved)

A saved theme pinning a stale preset (S620), the four layers a token lives in (S627), and the three accent tokens (S739): `.claude/rules/design-tokens.md` (auto-loads for `index.css`, `ThemeContext.js`, `Settings.js` and `DESIGN.md`).
