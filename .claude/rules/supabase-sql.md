...
- **`IF NOT <check> THEN RAISE` fails OPEN, because `is_admin()` returns NULL rather than false (S630).** The bullet above is the rule this project has followed since S293, and for seven weeks it prescribed a guard that does not guard. `is_admin()` is `select role = 'admin' from profiles where id = auth.uid()` — a `LANGUAGE sql` scalar, so **zero matching rows returns NULL, not false**. `NOT NULL` is NULL, `IF NULL THEN` never fires, and execution falls straight through into the privileged body. Every other operand behaves the same way: `p_client_id = my_client_id()` is NULL for a caller with no profile, and `false OR NULL` is still NULL.
  ...
  **Wrap the whole condition, do not harden `is_admin()`.** Making the helper return false would not close `get_cooccurrence` or `get_hr_self_service_status`, whose second operand is independently NULL for the same caller — they would keep falling open while looking fixed. Wrapping catches every NULL source at once, and changing `is_admin()` would alter how it evaluates inside ~120 policies where NULL and false are already indistinguishable. `set_active_outlet` (S617) is the one place that got this right unprompted; **`get_pos_device_secret` was recorded here as "safe" and was not (found S754).** The client check before it was NULL-safe, and that is as far as the reading went. Its RANK check, `IF NOT (is_client_owner() OR caller_pos_role = 'manager')`, is `false OR NULL` for an IMS manager, an HR manager or a Self-Service login of the same client, so the check never fired and any of them could fetch the till's device secret. COALESCE'd in `20260916110000`. **A guard with two conditions is two audits** — a NULL-safe first test says nothing about the second.
...
- `pos-staff-login` (`supabase/functions/pos-staff-login/`) is the till's PIN sign-in, `{ client_id, device_id?, device_secret, staff_id, pin }`: the device gate first (a per-tablet key through `verify_pos_device`, or the pre-S754 shared key through `verify_pos_legacy_device`), then the lockout, then the derived password. The device-key model (`pos_devices`, `register_pos_device` / `list_pos_devices` / `revoke_pos_device` / `get_pos_device_staff` / `retire_pos_legacy_device_key`, migration `20260916120000`) is in `.claude/rules/auth-and-pins.md`.
...
- **POS rank is fenced by TRIGGERS, not policies (S754, `20260916100000` / `20260916110000`).** The POS tables' same-client policies are `FOR ALL`, and a rank rule there is usually about which COLUMNS changed (a table's `status` is till work, its name is setup work), which a policy cannot see. So each table carries a SECURITY INVOKER BEFORE trigger calling the one shared test, `pos_caller_has_rank(level)`, with the `current_user NOT IN ('anon','authenticated')` seam letting DEFINER bodies and the service role through. The per-table map and the closed-bill lock are in `.claude/rules/pos-billing.md`. **A new POS table written from a ranked screen gets a trigger calling that function** — never a fourth copy of the rank test, and never a policy alone.
- **S756 stages 2–3 added, all applied live after rolled-back dry runs:** `save_purchase_bill` refuses a
  bill with returns and writes the optional invoice VAT/total columns; `ims_gate_pass_void_guard`
  (supervisor+, final, `voided_by` stamped); the supplier-credit pair (`payable_payments_credit_pair`, a
  DEFERRABLE INITIALLY DEFERRED constraint trigger — **test such a trigger with `SET CONSTRAINTS ALL
  IMMEDIATE` inside the rolled-back block, or it never fires**); requisition attribution stamped by
  `ims_requisition_attribution`, a final `rejected` status, and issued-slip line inserts limited to the
  issuer within 5 minutes below supervisor; the `dish-photos` bucket; and `logos_select_admin`.
  Stage 4 (`20260918170000`) rebuilt `get_group_summary` / `get_group_pnl` from their LIVE bodies
  (`pg_get_functiondef`) to count payslip overtime — rebuild a function from the live definition,
  not the last migration file, so an out-of-band change is not silently reverted.
- **IMS got the same shape in S756 (`20260918100000`): `ims_caller_has_rank(level)`**, which also refuses a count PIN (`ims_email`) at every rank, and the generic `ims_rank_guard(need, what[, update_need])` trigger. On: `purchase_entries` (staff; UPDATE manager — only `paid_at` is ever updated), `vendor_returns` (staff), `payable_payments` and posting depreciation (manager), `par_levels`, `recipe_ingredients`, asset categories and repairs (supervisor); bespoke guards on `requisitions`/`requisition_lines` (an issued slip needs supervisor), `stock_movements` (manual = IMS staff; `pos_sale`/`pos_comp` INSERT = POS or IMS supervisor; no UPDATE; non-manual DELETE = Owner/admin), `recipes` (`guard_recipe_rank`: hide supervisor, delete manager, with a 15-minute empty-recipe exception for Recipe Import's cleanup), `assets_register` (disposal columns manager) and `monthly_periods`; plus `ims_settings_rank` for the threshold columns inside `settings_guard_staff_roles`. The closed-month lock is `closed-periods.md`. **A new IMS table written from a ranked screen gets `ims_rank_guard`**, and its rank must match the page's — the S756 inventory found every rule by listing the writers first.
- **Money tables are fenced at MANAGER rank for writes (S751, `20260914210000`)**: `hr_payroll_runs`, `hr_payslips`, `hr_salary_components`, `hr_final_settlements`, `hr_advances`, `hr_advance_repayments`, `hr_festival_allowances`, `hr_incentives`, `hr_incentive_configs` carry `<table>_write_rank_*` requiring admin, Owner or `hr_role = 'manager'`; `hr_tada_claims` and its items carry the supervisor ones plus `no_hr_staff_rank`. A refused RLS UPDATE is 0 rows and no error, so a write that matters `.select('id')`s and checks the count. `monthly_periods` DELETE is admin/Owner only (`monthly_periods_delete_owner`) — IMS count-PIN and POS PIN logins could delete a month and cascade its payroll.
- **READS of pay are fenced from HR staff rank (S750, `20260914200000`).** RESTRICTIVE `no_hr_staff_rank` (FOR ALL, `is_hr_staff_rank()`) on `hr_employees`, `hr_salary_components`, `hr_payslips`, `hr_payroll_runs`, `hr_final_settlements`, `hr_advances`, `hr_advance_repayments`, `hr_festival_allowances`, `hr_incentives`. Supervisors keep reading basic pay and allowances (Overtime and the Roster's labour cost need them). **A new table holding pay joins this list**, and a page that renders a count from one of them below supervisor must hide the figure rather than show the RLS-empty zero.
