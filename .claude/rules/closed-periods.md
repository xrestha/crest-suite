...
- **Closing a period runs `closingCountPreflight()` first (S613).** It informs and never blocks, and counts the same `physical_qty IS NOT NULL` rows the carry-forward uses. Full reasoning in `.claude/rules/owner-report.md`.
