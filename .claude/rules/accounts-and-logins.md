...
paths:
  ...
  - "src/pages/Login.js"
  - "src/pages/Signup.js"
  # S770: narrowed from `adminClients/**`; of that directory only the drawer's Users tab manages logins.
  ...
  # Added S761. `/stock` is the ONLY page a count PIN can open, and the rule about what that
  # account may see lives here — yet this file loaded for the login screen and not for the page.
  # Same shape as the S713 note in ims-figures.md: every glob above matched real files, so
  # check-rules-globs had nothing to report while the set was missing the page the rule is about.
  - "src/modules/ims/stockcount/Stock.js"
---
...
**IMS is now the one module with TWO doors, and which one an account uses is a property of the ROW, not of the module.** An IMS staff account created with an email and password signs in at `/login`; one created from IMS Staff → Count PIN carries `profiles.ims_email` (synthetic, never returned to the browser) and signs in at `/ims/count`. Both hold `ims_role`, so `is_ims_staff()` and every RESTRICTIVE policy treat them identically — the difference is only the credential and the reach. Three consequences worth knowing before touching that screen:
...
- **Fencing the ROUTE left the PAGE untouched, and nobody noticed for 24 sessions (S761).** `/stock` is the one page a count PIN can open, and inside it the tab bar was the full seven — Opening Stock, Closing Stock, Wastage, Daily Wastage, Staff Meals, Summary, Print Sheet — because `TABS` had only ever been filtered by feature and by manager rank. It took a photo from a counter's phone (three wrapped rows of tabs before the first item) to surface it, and the clutter was the least of it: **Opening Stock was the DEFAULT tab**, and it is a live entry grid with its own Save All over the month's starting basis; **Summary** carried COGS, purchase value and an Excel export of the client's cost base; and Print Sheet + Summary between them made `ims_count_blind` readable in two taps, on paper in the Print Sheet's case. A count account now gets **one tab, Closing Stock**, with no NPR value per line, and blind count covers all three screens (`Stock.js`, the `TABS`/`blindOn`/`hideValues` block). **The general rule: a route allowlist says which pages an account opens, and says nothing about what is inside them — audit the page's own tabs, sub-views and export buttons separately.** Same family as `CLAUDE.md`'s "a nav allowlist is not a guard" and the S647 sub-route rule.
- **`imsCountOnly` was false for every session from S737 to S756**, because `fetchProfile()`'s select list did not include `ims_email` — the column the predicate reads. A tablet PIN reached every page an IMS staff login does. **A predicate over a profile column needs that column in the one select that fills `profile`**; this is the second time (after `active_client_id`, S617) a guard read a column nobody fetched.
- **Its rank is fixed at `'staff'` server-side.** A supervisor or manager count PIN would be a rank that unlocks pages the account cannot reach, so `create_ims_pin_staff` sets it and IMS Staff hides the role picker in that mode. Since S756 `update_ims_role` refuses any other rank for an `ims_email` target (admin included) and IMS Staff renders a PIN row's role as "Counting PIN" and leaves it out of both job-title re-rank paths — before that, a manager-rank PIN passed `ims_can_manage_counts()` and `isImsPrivileged`. `ims_caller_has_rank()` also refuses a PIN at every rank, and `ims-staff-login` refuses a settlement-blocked leaver. The shared tablet key is revoked with **Stock Count → Settings → Sign out all counting tablets** (`rotate_ims_device_secret`) and by `revokeClientTablets` on archive/clear/delete; `get_ims_count_staff` RAISES `ims_device_not_active` on a dead key rather than returning an empty roster. `get_ims_staff_list` returns `has_pin` so the row offers Reset PIN or Reset Password against the right credential, and returns NULL rather than the synthetic address for a PIN account — nobody types it, and it is half a credential.
...
**A login moved between clients arrives as an Owner, and leaves its outlet selection behind (S736).** Admin → Clients → Create User with an email that already has a client login offers to *move* it. That upsert wrote `client_id` and `role` and nothing else, so the login kept every column that pointed at the old client: `active_client_id` — which `my_client_id()` and `AuthContext` both resolve BEFORE `client_id`, so a grouped Owner who had switched outlets and was then moved kept reading and writing the old outlet under the new client's login — plus `hr_employee_id` (an employee of the old client) and the three staff-role columns, which made the moved login STAFF at the new client rather than its Owner, because Owner is the absence of markers. The upsert now clears all of them, the confirm says so, and migration `20260910140000` puts the `active_client_id` clearing (plus the login's `profile_outlet_access` rows) in a `BEFORE UPDATE OF client_id` trigger, so no future writer has to remember. Nothing else in the product changes a profile's `client_id`.
