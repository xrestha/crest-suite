...
- **The header's weather strip (S785, `WeatherStrip.jsx`) is for every client with the weather and a city, any module.** "The weather" is `hasWeather` (S786: the Growth key, or the POS or HR module), derived once in `useWeatherStrip` and rendered by `WeatherHeaderSlot` on BOTH the main Dashboard and the HR Dashboard (both of its returns); only the rain adjustment stays on `hasFeature('weather_forecast')` and needs IMS, through `salesForecastInputs`. With no city, the slot offers "Set your city" to a login `canEditWeatherCity` allows (Owner, admin, any module's manager — the database's `weather_city_rank`), and "Change city" under the days once one is set. Its words come from `weatherLook()`, whose "Rain" starts at `RAIN_MM`, and its "×N%" tags are exactly `rainForecastDays`, so the strip, the line and the footer cannot disagree about which days dip; the tags show only to a viewer who sees that chart. The strip is ready only when one of its own four days has a row (`stripHasForecast`); otherwise it says "Weather unavailable right now", never four blank days. Today's rain and high/low stay the pre-opening forecast; only its sky and thunder follow later fetches. `/settings` is an IMS route, so outside IMS the city is set from POS Setup → Weather or the dashboards' box (`WeatherCityPicker`, one component).
