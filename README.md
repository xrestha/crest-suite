...
Confirmed no other frontend code read `get_hr_self_service_staff`'s old shape (`SelfServiceLogin.jsx` was its only caller). Build compiles clean under `CI=true`, all 127 tests pass. The new Edge Function still needs `supabase functions deploy hr-selfservice-login` — a local build/test pass can't validate Deno-runtime code, and no Deno CLI is installed on this machine to check it standalone.
