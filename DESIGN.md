...
- **Don't** build a multi-series chart palette from the semantic tokens — on **Light**, `red`/`amber` collapse to ΔE 3.1 and `accent`/`red` to ΔE 5.6 under deuteranopia (floor 8). The original evidence — accent and purple being the *same hex* on Dracula, Catppuccin and Latte — died with those presets in S607; the rule did not, because the tokens are five *roles*, not five distinguishable hues.
