...
- **Accepted exception:** `.main-content`'s sidebar-collapse toggle animates `margin-left` (`Layout.css`, `transition: margin-left 0.22s ease`) rather than `transform`/`opacity`. Normally a layout-property animation, flagged as such. Kept as-is (confirmed 2026-07-12): `.sidebar-wrap` is `position: fixed`, so that margin is what carves out real space for it, not decoration - a `transform`-only fix would mean restructuring the sidebar's positioning strategy app-wide. The animation only fires on a rare, manual, user-triggered toggle, not a continuous or scroll-linked one, so the real jank risk is low. Revisit only if the sidebar's positioning mechanism changes for other reasons.
