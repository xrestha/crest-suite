              ...
              {moduleOpen('pos') && (
                ...
                },
                {
                  ...
                  tips: [
                    ...
                    '▦ QR on each table card generates that table\'s guest-facing digital menu — a read-only, no-login page a customer sees after scanning it on their own phone. Print it and place it on the table. Add a description, photo URL, and Veg/Non-Veg tag per dish in Recipes to make it richer; nutrition facts appear automatically if your plan has Nutrition Facts enabled',
                    ...
                    'Delivery Partners: set your negotiated Foodmandu/Pathao commission % in the Delivery Partners tab — applied automatically whenever staff selects one of them as the payment method at Charge, so Sales Report can show what the platform withheld vs. what you actually received. No live order sync with either platform yet (that needs a real API partnership) — this just tags a bill as coming from one of them',
                  ],
                },
                {
                  ...
                  tips: [
                    ...
                    'Foodmandu and Pathao appear alongside Cash/Card/etc. as their own payment methods — pick one when a bill came in through that delivery platform. If a commission % is configured (Table Management → Delivery Partners), the Charge screen shows what the platform withholds and what you actually receive; this only tags the bill, there\'s no live order sync with either platform yet',
                ...
                },
                {
                  ...
                  desc: 'Nine views of the same POS sales data, one page: Daily (day-by-day totals), Hourly (revenue by time of day), Bill Register (every individual voucher — payment mode, remarks, who closed it), Comped Bills (every bill that had an item comped out of it, cross-referenced to its NC number), Payment Summary (revenue by Cash/Card/eSewa/Khalti/FonePay/Credit), Category Wise and Item Wise (what drives revenue), Customer Wise (who\'s buying and how much), and 1L+ Report (Nepal VAT Annexure 13 — parties whose cumulative transactions exceed NPR 1,00,000 in a fiscal year). Requires Manager role or above.',
                  tips: [
                    'Daily/Hourly/Bill Register/Comped Bills/Payment Summary/Category/Item/Customer share one BS date-range filter; 1L+ Report uses its own Fiscal Year selector instead, since Annexure 13 is a whole-year compliance check, not an arbitrary range',
                    'Payment Summary groups the same VAT-ready Gross/Discount/Taxable/Net breakdown by how the bill was paid, so it reconciles against Daily and Bill Register totals for the same range',
