  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        tips: ['Recompute weekly for the freshest prediction — it only reflects sales up to the last time you ran it', 'A gold holiday badge reading "×N" means the forecast on that row IS scaled for the holiday; an amber ⚠ badge means the holiday is known but has no multiplier, so the row is unadjusted and should be read as a floor rather than a ceiling. The two used to look identical', 'A thin or brand-new POS history produces a less confident forecast — give it a few weeks of data', 'Set a Demand Multiplier on a holiday in Holiday Calendar (e.g. 0.3 if you close/run quiet, 1.5 if it\'s your busiest day) to have this page actually scale the forecast for it, not just flag it', 'Add movable holidays (Dashain, Tihar) to the Holiday Calendar so they get flagged even before you set a multiplier for them']
      },
    ]
  },
]
...
const HR_FEATURES = [
  ...
  },
  {
    ...
    tips: [
      ...
      'Cost % on that tab is banded and marked, not just coloured: ✓ at or under 30% of forecast revenue, △ from 30 to 37%, ▲ above it. Those are the same thresholds and the same marks the Owner Dashboard and the Monthly Owner Report put on Labour Cost %, so a day that reads healthy here reads healthy there',
...
const POS_FEATURES = [
                ...
                },
                {
                  ...
                  tips: [
                    ...
                    'Guests expected by hour is compared against the room\'s seats and only WARNS — the ninth booking for a forty-seat room is still yours to take. Next 7 days lists which days have an hour over seats',
                    'Online requests wait in an amber band at the top for Accept or Decline; the guest\'s phone shows the answer within seconds. From anywhere else in the app the count sits on the Reservations row in the sidebar and lights the POS dot in the rail (refreshed every minute), and the dashboard\'s Bookings Tonight tile says how many are waiting. Switch the link on and print its QR under Tables → Reservations',
          ...
          <div className="card" style={{ marginBottom: 16 }}>
            ...
            {[
              ...
              <div key={s.step} style={{ display: 'flex', gap: 14, marginBottom: 12, paddingBottom: 12, borderBottom: i < arr.length - 1 ? '1px solid var(--theme-border-lt)' : 'none' }}>
                <div style={{ width: 26, height: 26, borderRadius: '50%', background: 'var(--theme-bg)', border: '1px solid var(--theme-border)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, color: 'var(--theme-accent-ink)', flexShrink: 0 }}>{s.step}</div>
                <div>
