    ...
    features: [
      {
        ...
        tips: ['COGS and FC% are only accurate after closing stock is entered', 'If no open period exists, go to Periods and create one first', 'Dashboard figures always reflect the current open period', 'The "Daily Purchases vs Sales" chart overlays daily purchase spend (gold) with daily sales revenue (green). The green sales line only appears when you record sales day-by-day (Sales → pick a day); bulk monthly sales entry has no daily breakdown to plot. With 5+ days of data in the current month, dashed lines project both revenue (purple) and purchase spend (red) to month-end (trend estimates) — click the expand icon for the full view with running totals. Note: the gap between the lines is buying-vs-selling cash rhythm, not profit, and a purchase projection is inherently noisier than sales since restocking happens in bursts rather than a steady daily trickle.']
      },
      {
