    ...
    features: [
      {
        ...
        tips: ['In your first weeks, "Getting started with Crest" at the top lists the setup steps for the parts of Crest you use. A box ticks by itself once the job is really done — the first item saved, the first bill printed — not when you just open the page. Skip any step your business does not need, and "Hide for now" shrinks the list to one line. Been using Crest a while? Start it any time from Help → Getting Started, or "Setup guide" in the menu under your name (top right) — the steps you have already done are ticked', 'Fixed Costs % and Est. Net Margin use your finalized payroll for labour when a payroll run exists, otherwise what was typed on the Overheads Labor tab — never both — and the tile says which it used', 'COGS and FC% are only accurate after closing stock is entered', 'If no month is open, contact Crest support: Crest opens your first month when your account is set up, and after that each new month opens when you close the one before', 'Dashboard figures always reflect the current open period', 'The "Daily Purchases vs Sales" chart overlays daily purchase spend (gold) with daily sales revenue (green), with the BS weekday initial (S/M/T/W/T/F/S) shown below each date. The green sales line only appears when sales are recorded day-by-day (Sales → pick a day); bulk monthly sales entry has no daily breakdown to plot. A dotted "Target" line in a distinct colour (blue for sales, orange for purchases) shows what a usual day looks like, learned from the 4 weeks before the month began, so it is there from Day 1. Each weekday gets its own figure, and each line follows its own week: sales are highest on your busiest trading day, purchases on your usual restock day. A weekday you normally buy nothing on counts as zero, so the purchase target is low there. The Target is set once and never changes for the rest of the period, so the month can be judged against it. A new client without 4 weeks of history gets its Target after its first 7 days, from those days. A dashed line in the same colour as each metric forecasts the rest of the month: your usual weekday pattern, adjusted to how this month is running against it. This month\'s own days carry most of the weight within about a week, so a festival rush or a slow patch shows quickly. Click the expand icon for the full view with running totals. Hovering a day shows how far that day ran off its Target: the arrow points where the day actually sits (▲ above the target line, ▼ below), while the colour says whether that was the good direction for that metric — for Sales, above target is green; for Purchases it is the reverse, because spending under your usual pace is the win. A grey ≈ means the day landed close enough to count as on target — within 2% of it, or NPR 100, whichever is more forgiving — so the coloured days are the ones that actually moved. Note: the gap between the lines is buying-vs-selling cash rhythm, not profit, and purchases usually peak on a different day from sales (many kitchens restock ahead of the week, not on the busiest day).', 'Growth plan and above: the dashed sales forecast can allow for rain. The Owner picks the outlet\'s city and says what a rainy day does to trade in Settings → Weather (for example 85 = a rainy day sells about 85% of a normal one). On the next 7 days the weather forecast calls rainy (5 mm or more between 5:45 am and 11:45 pm), that day\'s forecast is scaled by the figure, and the footer names the days. Rainy days already gone this month are allowed for too, so a wet week does not make the rest of the month look slow. Purchases and the dotted Targets never change with the weather. The footer also shows what your own sales say a rainy day does, once enough rainy days have been recorded (it needs 5 rainy and 5 dry days, counted from the day the weather was switched on). Weather data: MET Norway.', 'Growth plan and above, or any restaurant using POS or HR: with a city set, the top of the Dashboard shows today and the next three days: a picture, the high and low temperature during opening hours (5:45 am to 11:45 pm), and the weather in plain words (Light rain from 1 mm, Rain from 5 mm, Heavy rain from 20 mm, Storms when thunder is forecast; a drier day reads Sunny, Partly cloudy or Cloudy). A rainy day the sales forecast allows for carries a small "×N%" tag, and the line under the days says what it means (for example "×60% = rainy-day sales forecast"). Hover or tap a day for its date and the rain in mm. The Owner or any manager sets the city: "Set your city" on the Dashboard ("Change city" under the days once one is set), POS Setup → Weather, or the Owner in Settings → Weather.','The "Spend by Category" card also shows the Top Items by Spend list — switch between the two with the tab at the top of the card.']
      },
      {
...
const POS_FEATURES = [
                {
                  ...
                  tips: [
                    ...
                    'Each till tablet has its own key. A manager or the Owner activates a tablet once from Till Devices (sign in with email on that tablet, open POS → Admin → Till Devices, give the tablet a name such as "Front counter", press Activate). A tablet that is lost or sold can be revoked there without affecting the others',
                ...
                {
                  icon: '⊞', name: 'POS Setup', path: '/pos/tables',
                  ...
                  tips: [
                    'Change a table\'s status from the list on its tile (Available, Reserved, Occupied, Inactive) — no need to open the editor. Nothing changes until you pick one; the till sets Occupied and Available by itself as orders open and close',
                    'Use sections to group tables by area; the section filter tabs appear automatically once you have more than one section',
                    'The Weather tab sets the town the outlet\'s weather is read for; today\'s and the next three days\' weather then show at the top of the Dashboard. The Owner or a manager can change it',
                ...
                },
                {
                  ...
                  tips: [
                    ...
                    'Configure which categories go to KOT vs BOT in POS Setup → Ticket Routing',
                    ...
                    'Tap "+ Add note" under any order item to send a special instruction (e.g. "no onion") to the kitchen/bar — it prints indented under that item on the ticket. Preset chips from POS Setup → Quick Notes appear while you type',
                    ...
                    'If something is missing before a bill can close, the button says what instead of looking ready: "Choose a discount reason first", "Enter the buyer\'s name and phone first", "Choose a reason for the comped items", "Choose a reason to void". Pressing it takes you to that box, opening Items or Buyer details if it is folded away. Void asks once more before it cancels the order. The Payment button on an order not sent yet says to send or save it first, and Issue & Print on a Credit Note works the same way',
                    ...
                    'Discount on the Pay tab can be capped per staff member — set a Discount Limit (%) for a login in POS Staff and their entered discount is automatically capped at that %, however they type it (flat NPR or percent). A new login starts at 0% — no discount — until you set one; clear the box for no limit (always the case for the owner/admin login)',
                    ...
                    'Applying any discount makes buyer Name and Phone compulsory (not just optional) and requires picking a Discount Reason — gives an identifiable, audited record of who received it. Customize the reason list in POS Setup → Discounts',
                    ...
                    'The payment method you have picked is the filled (coloured) button; the others stay plain even when the mouse is over them, so what is filled is what will be charged',
                    ...
                    'Typing works on the till too: type the number of covers and press Enter to open the table, and press Enter in "Cash received" to confirm the payment. On the Billing screen, "Find a table…" narrows the list when a guest asks for their bill by table',
                    ...
                    'On a phone, the menu fills the screen and the order sits in a bar at the bottom with the TOTAL, Send and Payment always showing. Tap "▴ Order" to see and edit the lines, and "▾ Back to the menu" to add more. On any touch screen every button on the order screen is at least finger-sized, and a problem with a send or save is shown just above the Send button',
                ...
                },
                {
                  ...
                  tips: [
                    ...
                    'A bill tagged Foodmandu or Pathao (an amber badge next to the customer name) shows a Commission % field when you Settle it — pre-filled from POS Setup → Delivery Partners as a starting suggestion, confirm or adjust it to match what the platform actually remitted before picking the settlement method',
                ...
                },
                {
                  ...
                  tips: [
                    ...
                    'Shift History lists every past shift — click one to see its full frozen Z-report (called the closing report on screen). A variance reads ✓ Balanced in green, ▲ short in red or △ over in amber, the same on the count screen, the report and the history list',
                    'Every POS report (Sales, Covers, KOT Log, Exceptions) has Today, This month, Last month and Last 3 months beside its date pickers, in Bikram Sambat months',
                  ],
                },
                {
                  ...
                  tips: [
                    ...
                    'Staff role = waiters who take orders only. They cannot access POS Setup or reports',
...
const ADMIN_FEATURES = [
                ...
                },
                {
                  ...
                  tips: [
                    ...
                    'If the client has no tables set up yet, add one in POS Setup first',
                  ],
                },
                {
                  icon: '◷', name: 'Audit Log',
...
const HR_WORKFLOW_STEPS = [
  { step: 1, title: 'Confirm the period is open', desc: 'The current BS month should already be open — there is nothing to create. Crest opens your first month when your account is set up, and each later month opens when the one before is closed (with Crest IMS, press "End <month> & Start <next>" on the Dashboard banner once the month has ended). If the month you need is not open, contact Crest support.' },
...
const POS_SETUP_STEPS = [
  { step: 1, title: 'Set up your Tables', desc: 'Go to POS → Admin → POS Setup → ⚡ Quick Setup to batch-generate a floor plan in one click.', why: 'Orders and billing are organised by table — there\'s nothing to bill against without them.' },
  ...
  { step: 4, title: 'Activate each till tablet', desc: 'On the tablet itself, sign in with your email and go to POS → Admin → Till Devices → give it a name ("Front counter", "Bar") → Activate. Then Sign out, so your own login is not left open on the till: from then on the tablet opens on the staff PIN screen (right after signing out, press Staff Login on the sign-in page to reach it).', why: 'The activation is saved on that tablet only, so it has to be done on the tablet itself. A tablet that has never been activated shows no staff list at /pos/login, so nobody can sign in with a PIN. Each tablet gets its own key, so a lost one can be revoked on its own.' },
...
export default function Help() {
  ...
  return (
    <div>
      ...
      {activeSection === 'guide' && (
        <div>
          {/* Your own setup guide (S790), always in full here — the same list the dashboard card
              shows, with its ticks, and the way back to the dashboard after hiding it. Renders
              nothing for a login that gets no guide (staff rank, PIN logins). */}
          <SetupGuideCard surface="help" />
          {imsEnabled && (
          <div>
          ...
          <div className="card" style={{ marginBottom: 16 }}>
            ...
            {[
              ...
              { step: 2, title: 'Add your Vendors', desc: 'Go to Vendors → add all your suppliers.', why: 'A purchase can be saved without a vendor, but then it shows with no supplier name in the vendor reports and in Outstanding Payables, and Crest cannot warn you when the same bill is entered twice. Add your suppliers before entering purchases.' },
              { step: 3, title: 'Check your first Period is open', desc: 'Go to Periods — the current BS month is already there, marked open. Crest opens your first month when your account is set up, so there is nothing to create. If no month is open, contact Crest support.', why: 'All purchases, stock, and sales live inside a period (one BS month). Nothing can be entered without an open period.' },
          ...
          <div className="card" style={{ marginBottom: 16 }}>
            ...
            {[
              { step: 1, title: 'Check the new Period is open',    desc: 'Nothing to create: closing last month (step 9) opened this one. Check Periods shows the current BS month as open. If no month is open, contact Crest support.' },
              ...
              { step: 9, title: 'Close the Period',                 desc: 'Once the month has ended, press "End <month> & Start <next>" on the Dashboard banner (or on Periods) → confirm. This locks the month for staff (the account owner can still correct it) and opens the next one. Closing stock automatically becomes opening stock for next month.' },
            ].map((s, i, arr) => (
