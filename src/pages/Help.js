...
const IMS_FEATURE_TIERS = [
  {
    ...
    features: [
      ...
      },
      {
        ...
        guide: 'Your ingredient database. Every item needs a name, category, UOM (unit of measure), and rate. Items are always stored in their smallest unit — sauce per ML, salt per GM — because that is what recipe costing needs. You can still type the pack you actually bought (Purchase Qty 1000, Rate 500 for a 1 KG bag) and Crest works out the NPR 0.50 per GM for you; the green line under the form always says what will be saved. Set conversion factors if you buy in bulk packs but consume in smaller units (e.g. 1 CTN = 24 BTL).',
        tips: ['Use consistent UOMs — GM for solids, ML for liquids', 'Two ways to price an item, same result: type the pack (Purchase Qty 1000 + Rate 500) or type the per-unit price straight into "Price per unit". Leave Purchase Qty at 1 when your invoice already shows a per-unit price', 'The Rate (NPR) / UOM column is the price of ONE unit — the exact figure recipe costing and stock valuation use. A sub-paisa rate is normal for something bought by the thousand and shows extra decimals rather than a misleading 0.00', 'Set conversion factor if 1 case = 24 bottles etc. — that is also what tells the Purchase Bill whether its Qty column means cases or bottles', 'Deactivate items you no longer use — do not delete them', 'Editing many items in a row? The Edit Item dialog has ← Prev / Next → buttons that save the current item and jump straight to the next one in the list (in the order shown), with an "X of Y" counter — no need to close and reopen. (Same on the Vendors edit dialog.)']
      },
      {
        icon: '⊙', name: 'Vendors',
      ...
      {activeSection === 'guide' && (
        <div>
          ...
          <div className="card" style={{ marginBottom: 16 }}>
            ...
            {[
              { step: 1, title: 'Add your Ingredients', desc: 'Go to Item Master → add every ingredient you buy. Each item needs a name, category, unit of measure (UOM) and a rate. Use the smallest unit you actually cook with — sauce in ML, salt in GM — and either type the per-unit price, or type the pack you bought (Purchase Qty 1000, Rate 500) and let Crest divide it down.', why: 'Every purchase, stock count and recipe cost is priced off this per-unit rate. You cannot enter purchases without items.' },
