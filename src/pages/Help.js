...
const HR_FEATURES = [
  ...
  },
  {
    ...
    tips: [
      ...
      'Publish is per BS month — self-service employees never see a draft roster, only what you\'ve explicitly published',
...
'Record overtime in ONE place — either the attendance sheet\'s OT column or the Overtime module. Both are paid, so the same hours entered in both pay twice; the payroll register flags affected employees with an ⚠ OT ×2? badge', 'TDS (income tax) is computed automatically from the fiscal-year tax slabs using year-to-date projection — finalize earlier months first so each month builds on the last; you can still override a value while the run is a draft', 'SSF contributors get the 1% first-slab social security tax waived, so most staff under roughly NPR 83,000/month gross pay zero income tax', 'Use the Payslip button on any row to view and print an individual payslip']
  ...
  },
  {
    ...
    guide: 'Gives employees a PIN-based login (no email/password) on their own phone to view their own payslips, submit leave requests, check their roster, and request shift swaps — without needing access to the main admin app. Enable it per employee from Employees → Enable Self-Service, setting an initial PIN. Share the single login link via Employees → Copy Self-Service Link — the same link works for every self-service employee at your company; each picks their name and enters their PIN. An employee can only ever see their own data, never a coworker\'s — this is enforced at the database level, not just hidden in the UI. The Roster tab only shows a month once the admin has clicked Publish on the Roster page; "🔔 Enable Notifications" (in the header) lets that employee\'s phone/browser receive a real push — even with the app closed — when a roster is published or a shift swap they\'re involved in changes status.',
    tips: ['One shared link per company — copy it from Employees and send it to your team (e.g. via WhatsApp)', 'Leave requests submitted through self-service go through the exact same approval flow as leave submitted by an admin', 'Payslips only appear here once a payroll run is Finalized', 'A PIN gets locked for 15 minutes after 5 incorrect attempts, same protection as POS staff PINs', 'Re-run Enable Self-Service on an employee to reset their PIN', 'Request Swap on a scheduled day lets the employee pick a coworker + one of the coworker\'s scheduled days from the same published month; the coworker must Accept from their own Swap Requests list before it reaches the admin for final approval', 'Notifications require the employee to tap Enable Notifications once and grant the browser permission prompt — it persists across sessions on that device after that']
  },
]
