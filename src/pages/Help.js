...
const HR_FEATURES = [
  ...
  },
  {
    ...
    guide: 'Track salary advances (short-term, recovered in the next payslip) and employee loans (multi-month, with scheduled installments). Issue a new advance or loan from the top-right button — set the employee, type, issued date, total amount, and an optional monthly installment amount as a reminder. Click any row to open the detail panel: see the repayment history, a progress bar showing how much has been recovered, and buttons to record a repayment or mark the advance as settled. Repayments are not auto-deducted from payroll — use the installment amount as a guide and add the deduction manually in Pay Setup under Other Deductions for that period.',
    tips: ['Advance = short-term, typically recovered in full next payslip. Loan = multi-month with a fixed installment.', 'Set an Installment/Month amount as a reminder — it pre-fills the repayment amount when you record a payment', 'The progress bar in the detail panel shows how much of the total has been recovered', 'Repayments must be recorded manually here after adding the deduction in Pay Setup or PayrollRun', 'Use "Settle" to close out an advance — you\'ll get a warning if there is still an outstanding balance', 'You can delete an advance only if no repayments have been recorded against it'],
  },
  {
    ...
    tips: ['Generate computes one month\'s basic, pro-rated by months worked toward the festival (capped at 12 months)', 'Recent joiners are automatically pro-rated — a 6-month employee gets about half a month\'s basic', 'Daily/hourly staff default to 0 — enter a figure based on their typical earnings', 'Amounts are editable while the run is a draft; Finalize locks it as a permanent record', 'Export the bank file (generic Name / Bank / Account / Amount columns) to disburse the bonus', 'TDS is estimated for each employee at the marginal income tax rate — married employees use the wider married tax schedule (FY 2082/83 and earlier)', 'Net payout = gross − estimated TDS; the TDS must be deposited with the IRD separately']
  },
  {
    icon: '💰', name: 'Gratuity',
