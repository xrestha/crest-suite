  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        tips: ['The Food Cost % figure, the tint of the box around it and the sentence under it all read your Settings → Thresholds values — change the warning level there and all three move together', 'Gross minus Net is discounts plus returns, not returns alone. Vendor Report and Consolidated P&L net off exactly the same discount, which is what keeps the three pages\' COGS tied', 'A high purchase-based FC% vs actual FC% gap often indicates closing stock errors', 'Share this report with ownership every month end']
      },
      {
...
const HR_FEATURES = [
  {
    ...
    guide: 'The HR command centre. An Approvals row up top covers every staff submission waiting on you — pending Leave requests, pending OT entries, pending TADA claims (manager-entered or Self-Service-submitted), and shift swaps that have cleared the coworker\'s accept and now need your sign-off. Below that: live headcount (active + probation), estimated basic payroll per month, advances outstanding, and employees retiring within 180 days. The payroll section summarises the last finalized run: net payable, employee SSF (11%), employer SSF (20%), and the total SSF challan amount to deposit — with the deposit deadline (25th of the following month — the law gives 25 days after the month ends; late deposits attract 10% interest). All four approval queues are listed further down so you can click through to approve or reject without navigating away.',
    tips: [
  ...
  },
  {
    ...
    tips: ['Pick the day, set statuses, then click Save Day — only the staff you actually marked or typed into are saved; anyone you did not touch stays blank', 'All Present / All Off / All Holiday fill only the blanks — a day already marked (approved leave, an absence, anything typed) is left as it is, and a message says how many were skipped. Change those one at a time', 'Setting a day to Absent, Paid or Unpaid Leave, Off or Holiday empties its Start, End, Break, Hours and OT boxes and switches them off — a day that was not worked takes no hours or overtime, because payroll pays overtime from every row whatever its status', 'Once payroll for a month is finalized, that month\'s sheet is locked: an amber banner says so and nothing can be changed. Reopen the payroll run first if the month really needs correcting, then finalize again', 'If the roster or its shift types cannot be read, an amber banner says overtime is not being calculated: typing Start/End will not fill Hours or OT and Generate from Roster is switched off, so nothing is measured against the wrong length of day. Type the figures yourself or reload', 'Clear Day clears only the staff listed on the sheet — someone who has left keeps their days, which Final Settlement reads', 'By Employee tab: pick a person from the dropdown, fill in their whole month day-by-day, then Save Month — the quick buttons (All Present/Off/Holiday) and ⚡ Generate from Roster there apply to just that one employee, not the whole team', 'Enter a Start and End time to auto-calculate Hours and OT Hours — hours beyond that day\'s Roster-assigned shift (or 8h if the employee isn\'t on the roster that day) become overtime automatically; both fields stay directly editable afterward if the auto-calc needs a manual tweak',
...
'Overtime can be entered on the attendance sheet or approved in the Overtime module. Where a day has both, the approved entry wins and the attendance hours for that day are ignored — the same hours are never paid twice. Holiday OT at 2× is only available through the Overtime module', 'TDS (income tax) is computed automatically from the fiscal-year tax slabs using year-to-date projection — finalize earlier months first so each month builds on the last', 'You can type over a TDS figure while the run is a draft. A typed figure is kept when you Finalize — it counts as a deliberate override — and the ↺ button beside it puts back the calculated figure. A TDS that would take someone\'s pay below zero is refused', 'Anything else that changes income tax — an earlier month finalized late, an insurance premium entered in Pay Setup, a festival allowance or bonus paid — makes the draft out of date. Press Regenerate before you Finalize', 'SSF contributors get the 1% first-slab social security tax waived, so most staff under roughly NPR 83,000/month gross pay zero income tax', 'SSF is deducted only when an employee is BOTH marked SSF-enrolled and has an SSF registration number saved in Pay Setup — an enrolled employee with no number contributes nothing and is flagged "⚠ SSF no. missing" in the payroll list, because a contribution with no number cannot be filed on the SSF challan', 'The TADA column cannot be edited here. An Approved TADA claim is paid by the first payroll after approval once the trip is over (its end date is on or before the last day of the month) — a trip from 30 Bhadra to 2 Ashwin is paid in the Ashwin payroll, and never twice. It is added after TDS, on top of net pay, without running through tax or SSF (a 🔗 icon shows how many claims). To change the amount, change or reject the claim in TADA Claims, then Regenerate. Finalizing marks exactly those claims Paid (Payroll); Reopen puts them back to Approved', 'An advance or loan cut never takes take-home pay below zero — if pay after tax is less than the instalment, only what is there is taken and the rest stays owed for later salaries. A fixed deduction such as CIT is cut the same way in a short month (for example someone who joined on the 25th)', 'Use the Payslip button on any row to view and print an individual payslip — while the run is still a draft it prints stamped "Draft — not final", so nobody mistakes it for the real one', 'The Excel export includes Department, Status, Unpaid Days, Worked Days, Hours Worked and Retirement (CIT) columns; a draft\'s file name ends in _DRAFT', 'The payslip header shows your company name, address, and PAN (from Settings) plus the employee\'s SSF number when they\'re SSF-enrolled — fill in Settings → Property Address/PAN if the header looks incomplete']
  ...
  },
  {
    ...
    guide: 'Computes, records and settles the full final payout when an employee resigns, is terminated or retires. Pick the employee, set their last working date (BS), and check the leave days, notice and festival lines — most of which the page now fills in for you from what is already in the system. It computes partial-month salary (gross pay, i.e. basic plus allowances, over the days in that BS month, less any unpaid days marked on the attendance sheet), leave encashment at basic ÷ 26 per day, gratuity if service has reached a year, pro-rated festival allowance if it has not been paid this fiscal year, minus unserved notice, outstanding advances and TDS on the lump sum. Finalize is the part that matters. It saves the settlement as a permanent record, writes a repayment against every advance the payout covers and closes them, marks the employee resigned/terminated/inactive with their last working date, and blocks their Crest Staff login — the three follow-ups that used to be a checklist you had to remember. The advance one is why it matters: an advance recovered in a settlement but left Active is deducted a second time on that employee\'s next payroll run. A finalized settlement is locked; an admin can Reopen it, which reverses every one of those writes, and past settlements are listed at the bottom of the page to reprint or mark as paid.',
    tips: ['Run this BEFORE marking anyone resigned or inactive — the settlement picker lists active and probation staff only, so deactivating someone first removes them from the page built for people who just left', 'Partial salary is GROSS (basic + allowances), not basic alone — the old calculator divided basic and silently dropped every allowance from the final month', 'If no attendance is marked for the final month, the page says so and prorates on calendar days instead of pretending it knows; mark attendance first if that month had absences, because after Finalize the employee leaves the attendance sheet', 'Unused leave days are filled in from the employee\'s balance for that BS calendar year — note leave runs Baisakh–Chaitra while festival and TDS on this same page use the Shrawan-start fiscal year', 'Gratuity\'s SSF offset now counts only the months SSF contributions were actually made, found from payslips — it used to assume contributions ran for the employee\'s whole service, which for a long-serving employee enrolled recently withheld a very large amount that was owed', 'TDS is the marginal rate above the employee\'s real year-to-date earnings, not a projected full year — a mid-year leaver was previously over-withheld', 'Finalize refuses if payroll already paid that final month, if a settlement already covers this employment spell, or if someone finalized the same settlement in another tab', 'A settlement that nets negative does not close the advances — that money genuinely has not been repaid, so the advance stays open', 'Finalized is not the same as paid: use Mark paid once the money actually leaves, so the record shows both', 'Monthly-salaried employees only — daily and hourly settlement is not supported yet'],
  },
  {
...
const FAQ = [
  ...
  { q: 'What happens to data when I close a period?', a: 'Closing a period locks it from further editing by your own logins, and freezes that month\'s Monthly Report. All data is preserved permanently and you can view closed period reports at any time. If a purchase bill turns up afterwards, your Crest admin can still enter it — Periods carries an "Add missing bills" action on every closed month that opens Purchases on that month.' },
