...
const HR_FEATURES = [
  ...
  },
  {
    ...
    guide: 'Daily attendance per BS month. The Mark Attendance tab lets you pick a day and set each employee\'s status (Present, Half-day, Absent, Paid/Unpaid Leave, Half-day Paid/Unpaid Leave, Weekly Off, Holiday), plus hours worked (for hourly staff) and overtime hours. Your configured Weekly Off Day (Roster → Shift Types, default Saturday) is auto-defaulted to Weekly Off; use the quick buttons to mark the whole team Present, Weekly Off, or Holiday in one click, or click ⚡ Generate from Roster to pre-fill the whole month from Staff Roster shift assignments in one step. The Month Summary tab shows a colour-coded grid of the whole month with per-employee totals, and exports to Excel. Attendance is the data source Payroll uses to calculate actual pay for daily and hourly workers and to apply overtime.',
    tips: ['Pick the day, set statuses, then click Save Day — each day is saved as a complete set for all active employees', '⚡ Generate from Roster fills blank days from that month\'s Staff Roster shift assignments (marked Present, with hours from the shift) and defaults unrostered weekly-off days to Weekly Off — it never overwrites a day you\'ve already marked, so re-running it after adding late roster entries is safe', 'A roster shift with no hours (e.g. a custom "LEAVE" or "Day Off" shift type some venues create just to mark the board) is marked Holiday by Generate from Roster rather than counted as Present — both are payroll-neutral, so no pay is added or deducted; use a formal Leave Request instead if the absence should show as Paid/Unpaid Leave', 'The weekly off day defaults to Saturday (Nepal standard) but is configurable per business from Roster → Shift Types — change anyone who actually worked', 'Half-day Paid/Unpaid Leave are written automatically when a half-day leave request is approved (see Leave) — Half-day Paid Leave costs the employee nothing, Half-day Unpaid Leave deducts half a day\'s pay', 'Public holidays are marked manually — use the "All Holiday" button or set Holiday per employee', 'The Hours column only appears for hourly-paid employees; OT hours can be entered for anyone', 'Overtime is captured here and paid at 1.5× the normal hourly rate during the payroll run', 'Only active and probation employees appear on the sheet']
  },
  {
    icon: '📋', name: 'Roster',
...
'Record overtime in ONE place — either the attendance sheet\'s OT column or the Overtime module. Both are paid, so the same hours entered in both pay twice; the payroll register flags affected employees with an ⚠ OT ×2? badge', 'TDS (income tax) is computed automatically from the fiscal-year tax slabs using year-to-date projection — finalize earlier months first so each month builds on the last; you can still override a value while the run is a draft', 'SSF contributors get the 1% first-slab social security tax waived, so most staff under roughly NPR 83,000/month gross pay zero income tax', 'Use the Payslip button on any row to view and print an individual payslip']
  ...
  },
  {
    ...
    guide: 'Tracks travel, transport, and daily-allowance expense reimbursements — actual itemized expenses, not a rate-based per-diem formula. Submit a claim with a trip purpose, destination, and dates, then add expense line items under Transport, Lodging, Daily Allowance, or Other with a description and amount each. A Manager approves or rejects the claim, then marks it Paid once reimbursed, recording the payment method. TADA is deliberately separate from Payroll Run — it is not taxable income and is meant to be paid promptly, not deferred to the next payslip.',
    tips: ['Add as many expense line items as needed per claim — the total is the sum of all items', 'Only Pending claims can be approved or rejected; only Approved claims can be marked Paid', 'Mark Paid records the payment method and timestamp for your own record-keeping — it does not touch payroll', 'TADA claims never appear on a payslip or feed into TDS — they are expense reimbursement, not salary', 'A Transport line shows a Vehicle picker (2-Wheeler/4-Wheeler/EV) and a Distance (km) field — Amount fills in automatically as Distance × that vehicle\'s configured Rate/KM. You can still hand-edit Amount afterward if needed (e.g. to add a toll).', 'Rate/KM itself comes from ⚙ Settings and can\'t be overridden here unless you\'re an owner/admin — if a vehicle shows "No rate set," ask an owner/admin to configure it, or just enter Amount manually.', 'The ⚙ Settings button (owners/admin only) sets each vehicle\'s Rate/KM and the list of Purpose options offered when starting a new claim — Purpose always has an "Other" choice for a one-off trip.', 'Start Date and End Date default to today when you open + New Claim — change them if the trip already happened or is scheduled ahead.']
  },
  {
