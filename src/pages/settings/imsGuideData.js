    ...
    sections: [
      {
        ...
        fields: [],
        formulas: [
          'Nearly every "food cost" figure in this module reduces to some version of: COGS = Opening Stock + Net Purchases − Wastage − Staff Meals − Closing Stock, where Net Purchases = Gross Purchases − each bill\'s discount (spread across that bill\'s own lines by allocateBillDiscounts) − Vendor Returns.',
        ...
        gotchas: [
          'Different reports compute "food cost" slightly differently on purpose — Overheads.js uses pure purchase spend (no opening/closing adjustment), Monthly/Annual Summary and Period Comparison use the full COGS formula above, and Budget vs Actual compares against net purchases only (no stock or wastage adjustment) — so these can legitimately show different percentages for the same month. This is the single most common source of "why don\'t these two pages match" support questions — see the callout on Overheads and Monthly Summary below. What is NOT a legitimate difference: every one of those pages nets the bill-level discount off purchases the same way, so their Net Purchases for one month agree to the rupee. Until S720 three of them did not. As of S747 the main Dashboard, Owner Dashboard, Monthly Owner Report and Group Console take bill discounts off purchases and food cost too — before that they charged the undiscounted price, so the same month read higher on the dashboards than on the reports.',
          ...
          'Every quantity and rate box in Purchases and Stock Count — and both boxes on the "Bought a pack?" line in Item Master — accepts arithmetic, not just a plain number — "3*24+7" commits 79. Alt+C opens a standalone Quick Calculator over any page. Worth mentioning to anyone still counting stock with a phone calculator in the other hand, since re-typing a total is where transcription errors get in.',
        ],
        connections:
    ...
    sections: [
      {
        ...
        fields: [
          ...
          { label: 'Tooltip arrows on the actual rows', desc: 'Hovering a day shows how far it ran off its Target. The ARROW is literal — ▲ above the target line, ▼ below — because it has to agree with the line you can see. The COLOUR is the verdict, and the two metrics have opposite polarity: for Sales, above target is green; for Purchases the reverse, because spending under the pace you locked in is the win. The percentage beside it is the gap as a share of that day\'s target. A grey ≈ replaces the arrow when the day landed within tolerance of target — 2% of it or NPR 100, whichever is more forgiving — because without a dead zone every single day earns a verdict, and a chart lit green and red on all thirty says nothing at all. Two thresholds rather than one: 2% of a big delivery day is fairly ignored, but on a quiet Sunday even a 10% swing can be NPR 20.' },
        ],
        formulas: [
          'Food Cost % = Net Purchases (after bill discounts and returns) ÷ Revenue × 100. Until S747 this page took purchases before the bill discount, so it read higher than Monthly Summary for the same month. The band is the CLIENT\'s own fc_warning_pct / fc_critical_pct from Settings — 35 and 45 are only the defaults — and it carries a shape marker (✓ Healthy / △ Watch / ▲ Too high) as well as a colour, so the verdict survives colour-blindness and a black-and-white print.',
        ...
        gotchas: [
          'On a lower plan the Growth+ panels show an upgrade card instead of a figure. For Top Variance Items, Menu Health and Items to Reorder the calculation itself is skipped, so a Starter browser never holds those numbers. The overheads read is NOT skipped — it runs on every plan — but a Starter client only ever sees the Fixed Costs & Net Margin upgrade card, which names Growth (the plan Overheads is sold on).',
      ...
      },
      {
        ...
        workflow: [
          'Client view: a table of the client\'s own periods, newest first. The "+ New Period" button (pick BS Year + BS Month) and the Close / Reopen / Resync actions on each row are shown to ADMIN only — a client\'s own login sees the list, "Show Archived" and (with POS) "Post POS bills to Inventory", and closes its month from the Dashboard\'s "End <month> & Start <next>" banner once the month has ended. Since S738 a period created by hand carries the previous EXISTING period\'s closing count into its opening stock, exactly as Close & Start Next does — and the notice says which month it came from, or that there was nothing to carry.',
          'Admin "all clients" view (no client selected): one row per property showing its currently open period, an EXPIRED flag if that month has passed, and inline Close & Start Next / End Period / Rename actions. A property with no open period gets "+ Create Period" (today\'s BS month, with carry-forward) — or, when today\'s month is already on record as closed, a "Reopen <month>" button behind a confirm, because that is what the click actually does.',
          ...
          '"End Period" (admin only, all-clients view) closes the current period without opening a new one — blocks further entry until someone creates the next period manually. When that happens months later, the created month is TODAY\'s, so the client\'s list has a gap; the carry-forward on create and the Resync button both walk the list rather than assuming month + 1, so the gap is harmless.',
        ],
        fields: [
          { label: 'Rename (open periods, admin)', desc: 'Changes the BS year/month of the OPEN period. Everything recorded in it — purchases, sales, stock counts, overheads, requisitions, attendance — is keyed by period id, so it moves with the rename and is reported under the new month; nothing is recomputed. Confirmed through the shared modal since S738, and audited (log_audit used to skip every monthly_periods update that was not a status change). For correcting a period created under the wrong month, not for moving figures.' },
          { label: 'Resync Opening Stock →', desc: 'On a closed row: re-runs the close-time carry-forward from this month\'s closing count into the NEXT PERIOD THAT EXISTS for the client — not month + 1 (S738). Overwrites the target\'s opening stock for every item that has a closing count here; items without one are left as they are. Safe to repeat.' },
          { label: 'Post POS bills to Inventory', desc: 'Posts POS bills that closed while no Inventory period existed for their date. Since S747 it also posts CREDIT NOTES issued in that month that could not take their bill\'s revenue back out of Inventory at the time (because no period was open) — each goes in as a negative revenue line, after the bills, and stock is not touched because the food was served. The confirmation names both counts ("Post 3 bills and 1 credit note"). Safe to run more than once; anything already posted is skipped. Admin and the account Owner see it on every row; everyone else only on the OPEN period, because it writes sales_entries and stock_movements and a closed month is read-only for every other login on every other page too (enforced in the database since S756). Posting into a closed month leaves its frozen Monthly Report stale — the notice says so and names Regenerate Snapshot.' },
          { label: 'BS Year / BS Month', desc: 'Nepali calendar; the form accepts 2070 up to the calendar table\'s last verified year (BS_YEAR_MAX in bsCalendar.js). Month 1 = Baisakh … 12 = Chaitra. Nepal\'s fiscal year runs Shrawan (month 4) through Ashadh (month 3 of the following BS year). Note the real day-length table covers BS 2000–2087 — a period created beyond 2087 falls back to a 30-day approximation, which is fine for a placeholder and wrong for anything dated.' },
          { label: 'Show Archived', desc: 'Closed periods older than 12 months are hidden by default; toggle to reveal them.' },
          { label: 'Add missing bills →', desc: 'On a closed row: opens Purchases on THAT month (/purchases?period=<id>) so a bill missed at the time can still be entered. Closed months are writable for admin and the account Owner (S756), so this replaces reopening for the case people actually reach for it — and unlike Reopen it works while a later month is open. Regenerate that month\'s Monthly Owner Report afterwards; the snapshot was frozen at close and does not include the new bill until it is.' },
          { label: 'Reopen', desc: 'Admin only. Sets a closed period back to open — which only matters for handing entry back to the client\'s STAFF logins, since admin and the Owner can already write to a closed month. No carry-forward logic re-runs; opening stock for that period already exists from when it was first created.' },
        ],
        formulas: [
        ...
        gotchas: [
          ...
          'Neither admin nor the account Owner needs to reopen a period to edit it — every period-scoped entry page exempts both from the closed-period lock, and since S756 the database enforces the same pair (ims_closed_period_guard on counts, wastage, staff meals, sales, purchases, returns and requisitions). Settling an old bill on Outstanding Payables is the one change allowed for other logins. Edit in place, re-run "Resync Opening Stock" if a closing count changed, then Regenerate Snapshot on that month\'s Monthly Report.', 'Who may change a month is enforced in the database too (S756): creating or closing a month needs the Owner or an IMS supervisor/manager; reopening or renaming needs the Owner or admin. The Dashboard\'s End month button is shown only to those logins.',
          'Purchases opens on the OPEN period by default, which is why editing a closed month used to feel impossible even though it never was — the month simply could not be reached. "Add missing bills" and the ?period= link it uses are the way in; Purchases and the bill form both show an amber "you are editing a closed month" banner once you are there, so it can never happen by accident.',
          'Creating a period is not only an IMS event (S741). hr_attendance rows hang off a period, so leave approved for a month that does not exist yet has nowhere to be written — and payroll reads the attendance sheet, not the Leave page. Minting the month back-fills those days, and the create/close notice says how many. A failed back-fill is reported as its own stage rather than swallowed: the month is open either way, but until those days are marked an approved unpaid leave is not being deducted.',
        ],
      ...
      },
      {
        ...
        workflow: [
          'If no categories exist yet, a "Load Default Categories" button seeds 6 standard ones (Dairy & Bakery, Meats & Poultry, Groceries, Veg & Fruits, Beverage, Misc. Items).',
        ...
        fields: [
          ...
          { label: 'Conversion (Purchase Unit / Conversion Factor)', desc: 'Set only when you buy in one unit (e.g. CTN) but track in another (e.g. BTL). Both fields are required together or not at all — a partial conversion is rejected at save. Base Unit is shown but not editable: it is always the item\'s own UOM, because that is the unit stock is counted and valued in. Choosing a different one was never possible in practice and only ever mislabelled the price on screen.' },
        ...
        gotchas: [
          ...
          'The pack line is never remembered — reopening an item shows it blank, because there is no column to keep a pack size in. That is deliberate: buying in CTN and counting in BTL is described by the Conversion tab alone, which is what the Purchase Bill reads to decide whether its Qty column means cartons or bottles.',
          ...
          'Hiding an item is not purely cosmetic: a hidden item is left out of stock valuation and the monthly summary as well as the entry screens, so hide an item once its stock is down to zero rather than while it still holds any.',
          '"Used In" covers eleven tables — recipes, purchases, opening/closing stock, wastage, staff meals, requisitions, vendor returns, par levels, purchase orders and stock movements. If any of those cannot be read the column says "not checked" instead of showing a blank, and deleting is blocked: three of them would be deleted along with the item rather than stopping the delete, so an unchecked item is never safe to remove.',
        ],
      ...
      },
      {
        ...
        gotchas: [
          ...
          'Two of the four references would NOT be caught by the database. purchase_entries and purchase_orders hold a plain FK so Postgres refuses the delete, but vendor_returns and ims_gate_passes are ON DELETE SET NULL — the delete succeeds and those rows silently lose their supplier, and vendor_returns stores no name of its own. That is why the check is done across all four rather than left to the FK — and since S708 it is done in the DATABASE as well as the page: a BEFORE DELETE trigger on vendors refuses any referenced vendor, because the page guard only ever protected the page, and every IMS account can reach the table directly. There is deliberately no force-delete for a vendor: Archive already keeps every record intact, so the only thing a force-delete could do is destroy the purchase history that proves the supplier existed.',
      ...
      },
      {
        ...
        workflow: [
          ...
          'Five filters, all combinable and all reflected in the entry count and both footer totals: day pills, Item, Vendor, a Bill no. search (substring on the vendor\'s invoice ref, case-insensitive, a leading # optional) and Payment. The Payment list is built from the methods this period actually used, so it never offers an option that would return nothing, and it resolves a blank payment_method as Cash exactly as the row badge does. Changing period clears all five.',
          ...
          'Returns tab: pick an existing purchase line, enter a return qty in the same unit and the day the goods went back (pre-filled with the bill\'s day) — Rate, Vendor, and Payment Method are always inherited from the linked purchase, never independently set. Until S698 the return was silently dated to the bill\'s day, so Vendor Balance Confirmation\'s ledger showed returns before they happened.',
          '"Delete All" (purchases or returns) requires typing the exact period label to confirm — deliberately higher friction than a single-bill delete — and is Supervisor and above (S698). Staff keeps single-bill add/edit/delete.',
        ...
        fields: [
          ...
          { label: 'Entered <time> (under the vendor name)', desc: 'When the bill was keyed into Crest — purchase_entries.created_at — NOT when the goods were received. The Day column is the delivery day, and the two routinely differ, since the closed-period workflow exists precisely so a Shrawan bill can be filed in Bhadra. The entry DATE is therefore shown alongside the time only when it differs from the bill day, which makes a late-filed bill obvious without adding noise to the normal case. Editing a bill no longer restamps this: the save path replaces every line, so the original entry time is carried forward and one bill keeps one entry time however many times it is corrected. Nepal time, whatever the viewing device is set to.' },
          { label: 'Bill no. search', desc: "Matches purchase_entries.invoice_ref — the vendor's own bill/invoice reference, shown as #TII6339 under the vendor name on each bill row. Substring and case-insensitive, so the tail digits are enough; a leading # is stripped, so pasting the reference exactly as displayed works." },
          { label: 'Payment filter', desc: 'Cash / Credit / FonePay, restricted to the methods present in the selected period. A bill written before the payment_method column existed holds NULL and is displayed and filtered as Cash — filtering on the raw column would hide rows the screen labels Cash.' },
          ...
          { label: 'Quick Calculator (Alt+C)', desc: 'Press Alt+C while entering a bill for the app-wide calculator, floating over the form — useful for working out a per-carton rate mid-entry. The bill itself stays live underneath: you can read a rate off a line, type into the form, and drag the calculator by its title bar wherever it is not in the way. The bill screen used to carry a second ⌗ button of its own; that existed only because the dialog it sat in swallowed Escape before the calculator saw it, and it went when the dialog did.' },
        ],
        formulas: [
          ...
          'Net Purchases (used everywhere) = Gross Purchases − bill discounts − Vendor Returns. Gross and returns are Σ(qty×rate) in base units; each bill\'s one discount is spread across that bill\'s own lines in proportion to their value (allocateBillDiscounts), which is why Monthly Summary shows Gross, Discount and Returns as three separate columns. Since S747 the Dashboard, Owner Dashboard, Monthly Owner Report and Group Console use this same net figure.',
        ...
        gotchas: [
          'Editing a bill goes through the save_purchase_bill RPC (S698): the old lines are deleted and the new ones inserted in ONE transaction, so a failure mid-save leaves the bill exactly as it was — never holding both versions (which double-counted it in every purchase figure) and never empty. If another user changed the bill while it was open, the save is refused with "this bill changed" rather than overwriting their edit.',
          'A Purchase Order receipt is one bill: PurchaseOrders stamps one purchase_group_id across every line it receives (S698). Before that, purchase_group_id defaulted per ROW, so a six-line delivery showed as six separate bills on this page.',
          'The Gross Purchases figure here is qty × the rate on each bill, before discounts. Monthly Summary and P&L take the same bills net of their discounts, and Stock Count\'s Summary values what arrived at the Item Master rate — three legitimately different figures; none of them is wrong, and no tooltip claims they match. The Daily Register\'s Total column is a QUANTITY in the base unit, not money.',
      ...
      },
      {
        ...
        formulas: [],
        gotchas: [
          'The page needs IMS Staff rank or above — so any IMS login, including a Staff-rank one, can issue a pass, and there is no supervisor-only action on it. A POS-only or HR-only login (no IMS role) is sent back to the Dashboard.',
        ],
      ...
      },
      {
        ...
        workflow: [
          ...
          'Daily Entry tab: pick a specific BS day, enter qty per recipe for just that day (bs_day > 0). On a day the till already sold something (admin only, since the tab is locked for everyone else on a POS client) a read-only "From POS" column appears beside Qty Sold, with its own revenue line. Save Day writes only the typed manual figures and never touches a POS row. Before S699 those POS quantities pre-filled the editable box, so one Save Day handed them back as source=manual rows alongside the surviving originals and roughly doubled the day.',
        ...
        gotchas: [
          ...
          'Comps (source=\'pos_comp\', written automatically by POS) are excluded from every read here — manual entries from this page are never comps by construction. That exclusion is applied in JS over a selected source column, never as a server-side .neq on source: the column is nullable, so NULL <> pos_comp evaluates to NULL rather than true and every legacy NULL-source row was silently dropped. On this page that was destructive rather than merely under-reported. An invisible row is also absent from the save payload, and the save_sales_day delete covers source IS NULL OR source = manual, so the next Save Day removed a row nobody had ever been shown (fixed S699, pinned by salesReads.test.js).',
          'A discount typed with no quantity sold is refused at save time and the items are named (S699). The payload filter is qty > 0, so it used to be drawn on screen as negative Day Revenue, counted in the Total discount for that day, and then silently discarded.',
          'Arrowing the day picker starts one load per press, and the Daily grid is the baseline a save merges every untouched row against. The loader is now claimed by a periodId:day key (useLatestRequest), so a slower earlier load can no longer land the figures for one day under the date of another and have Save Day write them there (S699).',
          'init() dropped both of its read errors until S699. A failed periods read rendered NoPeriodState; a failed recipes read rendered the no-recipes empty state above a live Save button, and both payload builders iterate the recipe list, so an empty menu built an empty payload, which save_sales_day reads as delete-this-day-and-insert-nothing. Both Save buttons, both Clear buttons and Import Excel are now disabled on an empty menu.',
      ...
      },
      {
        ...
        workflow: [
          ...
          '"Receive" (Goods Receipt Note): re-reads the order\'s lines first, then shows Ordered/Received/Remaining per line, an editable Receiving Now qty, a Day Received picker locked to the order\'s own BS month, Payment Method (defaults to Credit), and a VAT-Incl. checkbox. If the read fails the screen does not open at all — a Remaining figure drawn from an error is the most expensive wrong number this page can show.',
          '"Confirm Receipt": one transaction (receive_purchase_order). It re-checks the remaining quantity under a row lock, refuses a closed period, writes one bill (one shared purchase_group_id) with every line pointing back at the PO, increments qty_received, and sets the status from the table. Either all of that lands or none of it does.',
          '"Cancel" on a draft/sent PO creates no purchase entries. On a part-received PO the button reads "Close Short": goods already received keep their bills and their stock, and only the outstanding quantity is closed off. Delete is operator-only and refuses an order that has bills against it — cancel that one instead.',
        ],
        fields: [
          { label: 'PO Number', desc: 'Auto-generated PO-{3 digits} from the current max, with a 3-attempt retry against a unique-constraint collision (two tabs open, fast double-click). The scan that finds the max is paged, so it stays right past 1000 orders.' },
          { label: 'Unit Price on the PO', desc: 'A pre-agreed price carried straight through to the resulting purchase entry — it does not re-check against the item\'s live Item Master rate at receive time. It is per BASE unit, so no conversion factor is applied anywhere on this page (unlike a Purchase Bill, whose qty is in purchase units). If "VAT Incl. (13%)" is ticked on the receive screen, the rate written to the bill is unit_price ÷ 1.13 — the PO price is treated as VAT-inclusive and stored ex-VAT, like every purchase rate.' },
          { label: 'Expected Delivery', desc: 'Picked in BS and stored as AD, like every date in the product — and now displayed in BS everywhere it appears, including the printed order the supplier receives. Before S709 the list and the print showed the raw AD value.' },
          { label: 'Day Received', desc: 'The day the delivery arrived, inside the ORDER\'s period — not necessarily this month. A Shrawan order received in Bhadra files its bill in Shrawan, and the screen says so. The picker is locked to that month, so it can only offer days that month actually has (BS months run 29–32 days).' },
        ],
        formulas: [
          'PO Total = Σ(qty_ordered × unit_price).',
          'Remaining = max(0, qty_ordered − qty_received), rounded to 3 decimals — the precision the column stores.',
        ],
        gotchas: [
        ...
        connections: 'Requires Vendors and Items. Confirmed receipts write directly into purchase_entries — the same table Purchases reads — so received goods show up everywhere a manual purchase would, and correcting one of those bills in Purchases keeps its link back to the order.',
      },
      {
        id: 'stock-count',
        title: 'Stock Count',
        ...
        workflow: [
          ...
          'A failed read renders the "Could not load" card under the page header and nothing else — no tabs, no Save All. That includes a failed FIRST load: until S747 it left the period list empty and the page said the client had no periods instead of showing the error. Before S695 the reads dropped their errors, every cell rendered blank, and Save All then DELETED the server\'s real rows for every visible item, since a blank means "no row".',
          ...
          'Settings tab (Growth, Manager only, S737): assigns sections to counters, switches on blind counting and recount protection, and shows the QR that sets a counting tablet up. Everything on it is off by default, so a client counts exactly as before until a manager opts in.',
        ],
        fields: [
          ...
          { label: '↩ Pull from last month', desc: 'Opening Stock tab only. Finds the chronologically-previous BS period, reads its counted closing_stock.physical_qty, and upserts those into this period\'s opening_stock — the same "closing IS next opening" rule the period-close path uses, but re-runnable at any time. Confirms before overwriting, carries every counted item — a count of 0 included, which clears this month\'s opening for that item (S695) — and needs to be online. Disabled on a locked period. Its read and its write are both checked: a failed read says so rather than "never counted", and "✓ Saved" only shows when the write landed.' },
          ...
          { label: 'Who counts what', desc: 'Settings tab. A grid of staff against categories: tick the sections each counter is responsible for. Supervisors, managers and the Owner always count everything and have no ticks. The master switch above it ("Limit counters to their assigned sections") is what makes it bite, and it is enforced by the database on every write to closing_stock, not by the screen — a counter cannot save a section that is not theirs by any route. It fails CLOSED: a staff-level counter with no ticks and the switch on can save nothing, which is why the tab warns by name when someone is in that state.' },
          { label: 'Blind count', desc: 'Settings tab. Takes the Purchased, Returned and Value columns off the Closing Stock tab for a staff-level counter, so they write what is on the shelf instead of confirming what the system expected. This changes what is SHOWN, not what the browser can reach — it is counting discipline, not a lock, and the tab says so.' },
          { label: 'Recount protection', desc: 'Settings tab. Once someone has counted an item, another staff-level counter cannot change or clear it; a supervisor, manager or the Owner still can. Enforced by a database trigger. Every closing count now records who entered it and when, whether or not this is on.' },
          { label: 'Count PIN accounts', desc: 'Created from IMS → IMS Staff → Count PIN: name plus a 4–6 digit PIN, no email and no password. They sign in at /ims/count by tapping their name, and reach Stock Count and nothing else. Five wrong tries locks the account for 15 minutes; a manager can reset the PIN from the same screen.' },
        ],
        formulas: [
        ...
        gotchas: [
          ...
          'A counting tablet is set up by SCANNING the QR in Settings, not by pressing a button on the tablet — a store-room device has no manager session to press it with. The code lasts 15 minutes and any number of devices can be set up in that window; "Hide code" ends it early. A device already set up keeps working after the code expires.',
          'A count PIN account is fixed at Staff rank, because it can only reach Stock Count — a supervisor or manager PIN would be a rank that unlocks pages it cannot open.',
          'Section assignments are NOT restored by a client restore. They key on the staff account, and a restore re-creates those accounts with new ids, so the grid is re-ticked by hand afterwards rather than being silently pointed at the wrong people.',
          'Only items with a closing_stock row carry forward. An item with no row is not carried at all — identical to never having entered it manually. Since S695 a counted 0 IS a row, so it carries forward as an opening of 0 (the period-close path already filtered on physical_qty IS NOT NULL, so it needed no change).',
        ],
    ...
    sections: [
      {
        ...
        workflow: [
          ...
          'Ingredient rows are each either an Item or another recipe tagged Sub-Recipe. A sub-recipe can\'t directly list itself; indirect cycles are checked at save time and rejected, for any recipe rather than only one still categorised Sub-Recipe (S714). The same ingredient listed twice is refused rather than added up — combine the two rows.',
        ...
        formulas: [
          ...
          'True Cost with Overheads (when the period has both overhead entries and sales revenue): overhead per portion = (Fixed Overheads bucket only × this recipe\'s revenue share of the period) ÷ portions sold; True Cost = ingredient cost + overhead per portion. "Covers sold" here has always meant portions, not guests — the Overheads page renamed the same figure to Dishes in S716.',
        ],
        gotchas: [
          ...
          'A sub-recipe that other recipes list as an ingredient cannot be deleted. The page refuses up front and names them, and the database refuses too (recipe_ingredients.sub_recipe_id is a plain foreign key), so it holds however the delete is attempted. Remove it from those recipes first, or use Hide — an inactive sub-recipe stays fully usable as an ingredient.',
          ...
          'The menu price and VAT of an EXISTING dish are a manager\'s (S754): below POS or IMS manager (and not the Owner) those two boxes are read-only here and are left out of the save, so a supervisor editing yield or ingredients never trips the price rule. A NEW dish a supervisor creates keeps the price typed, but the database puts it on the menu switched OFF for the till — a manager turns it on from Menu Pricing, where its price is on screen. Takes effect once migration 20260916100000 is applied.',
          'A recipe that has ever sold cannot be deleted — sales_entries.recipe_id is a plain foreign key, so Postgres refuses, and until S711 the page cleared the ingredient list BEFORE hitting that refusal, leaving the recipe alive and empty. Hide is the intended path: it removes the recipe from Sales Entry, POS, the QR menu and the menu reports while past sales keep their figures.',
          'Deleting a recipe is not one rule — the seven things referencing it disagree. sales_entries refuses; recipe_ingredients, demand_forecast_daily and recipe_suggestions cascade (their rows go with it); pos_kot_removals sets null; and pos_order_items.recipe_id has no foreign key at all, so POS bill lines would silently point at nothing. The page counts sales and POS lines up front instead of trusting the database to stop it.',
          'The recipe walk behind Variance, Reorder, Stock Report and both dashboards reads one recipe_ingredients row per ingredient across the ENTIRE recipe book. Until S711 it was unpaged, so past 1000 rows (~120 recipes at 8 ingredients) the recipes below the cut exploded to nothing — which reads as over-consumption, not as missing data. Figures from before that date can flag false variance and understate reorder need on a large menu.',
        ],
      ...
      },
      {
        ...
        workflow: [
          ...
          'IMS branch: type a new VAT-inclusive price in the New Price box and press Enter to save. POS-only branch: Price is shown as plain read-only text — change it through the row\'s Edit link, which opens the item form. Both branches: toggle On POS to control order-screen visibility (also how staff "86" an item that\'s run out).',
        ...
        gotchas: [
          'Who can open it (S754, decided with the owner): a POS manager, an IMS manager, the Owner or admin, on any client — by URL as well as from either module\'s menu. It used to ask only the IMS rank, and only on an IMS client, so a POS manager on a two-module client was refused while any POS login on a POS-only client could reach it by URL. The database enforces the same set on the price, VAT rate and On POS toggle (guard_recipe_menu_price, migration 20260916100000), so the rule is not only a page rule. The chip above is the plan; this is the rank.',
          'A price change here reaches only NEW lines on the till. A dish already on an open order keeps the price it was ordered at — the till prices every new line from the menu on the server, never from the tablet (S754).',
      ...
      },
      {
        ...
        fields: [
          ...
          { label: 'Dishes', desc: 'Portions sold (Σ qty_sold, comps excluded) — NOT covers. Crest counts covers as guests on a POS bill (pos_orders.covers, the Covers Report, Demand Forecast); this page reads sales_entries, which carries none. Was mislabelled "Covers" until S716, reading ~3x the real guest count.' },
        ],
        formulas: [
        ...
        gotchas: [
          ...
          'A bucket with nothing in it reads "not entered", never NPR 0 at 0.0% (S716). The traffic light bands 0% as GREEN — 0 is comfortably under every target — so an untouched Labor tab used to render as beating a 30% target, and Net Profit was overstated by whatever was missing. A caveat above the Net Profit callout now names every missing line.',
          'Revenue here excludes comps, and the filter runs in JS over a selected `source` column — never as .neq(\'source\',\'pos_comp\'), which drops NULL-source legacy rows too. Revenue is this page\'s denominator for every percentage while food cost comes from purchases, so a short revenue pushed Food Cost % and break-even UP and Net Profit DOWN, which is the sign that picks green "Profitable" vs red "Operating at a loss" (S716). Pinned by salesReads.test.js.',
        ],
      ...
      },
      {
        ...
        formulas: [
          ...
          'Tax pool depreciation (declining-balance): each pool\'s own statutory rate (A 5%, B 25%, C 20%, D 15%) applied to a shrinking pool balance, not per-asset. In the year an asset is bought, its share of the rate depends on when in the BS fiscal year it was acquired: Shrawan–Poush (the first six months) gets the full rate, Magh–Chaitra gets 2/3, Baisakh–Ashadh gets 1/3. The asset still enters the pool at its full cost — only that first year\'s charge is reduced. Pool E (intangibles) is straight-line instead: cost ÷ useful life, with a full year\'s amount in the first year if acquired Shrawan–Poush and half a year\'s if acquired Magh–Ashadh (Crest\'s reading of the law\'s "nearest half year" wording — verify with an accountant).',
    ...
    sections: [
      {
        ...
        formulas: [
          ...
          'Food Cost % = COGS ÷ Net Sales Revenue × 100, banded against the client\'s own fc_warning_pct / fc_critical_pct (defaults 35 / 45) with a ✓ / △ / ▲ marker beside the colour. The page no longer prints any fixed "industry target" range — the box, the figure and the sentence under it all follow the client\'s own thresholds.',
        ...
        gotchas: [
          'A category is only included if it had ANY activity. That test missed RETURNS until S720, so a category whose only movement this period was a return of goods bought in an earlier month was dropped entirely — its credit never reached the Returns KPI (which read "None this period") and COGS was overstated by the whole amount.',
          'The Gross Purchases column was fed the POST-discount figure under a pre-discount label until S720, so Gross minus Net read as "returns" when part of it was the bill discount, and the Net Purchases cell printed a dash on any bill that had a discount but no return. Gross, Discount and Returns are three separate columns now and Net Purchases always prints.',
          'The FC% figure was routed through the client\'s own thresholds in S682; the box around it and the sentence under it were not, and both kept a hardcoded 35/45. A client on a 30% warning level saw a red ▲ figure inside a green box captioned "✓ Within benchmark (28–35%)". All three read fcBand as of S720.',
        ],
      ...
      },
      {
        ...
        fields: [],
        formulas: [
          'Per month: COGS = Opening + Net Purchases (gross − bill discounts − returns) − Wastage − Staff Meals − Closing. Identical to Monthly Summary, Stock Count and Period Comparison — this page used to leave Staff Meals out (fixed 2026-08-13) and then, until S720, left the BILL DISCOUNT in, which put its COGS ABOVE Monthly Summary\'s for the same month by exactly the discount.',
        ...
        gotchas: [
          ...
          'Every read here spans 12 periods at once, so PostgREST\'s silent 1000-row cap bites at ~85 items rather than ~1000. Until S720 opening_stock, closing_stock and staff_meals were unpaged: whole months lost their stock, COGS collapsed to net purchases for those months, and — with no sort tiebreaker — WHICH months differed between loads. Truncation returns no error, so nothing flagged it.',
          'An item deactivated mid-year used to be counted at full purchase value while its opening/closing/wastage valued at 0 — an internally inconsistent row whose COGS was overstated by the closing value it had just discarded. It is excluded from every column now, matching Monthly Summary.',
        ],
      ...
      },
      {
        ...
        fields: [],
        formulas: [
          'Same COGS formula as Monthly/Annual Summary, staff meals and bill discounts included (the discount half landed in S720). Colour bands read the client\'s own fc_warning_pct / fc_critical_pct — the same values Monthly Summary uses — for the table cells, the chart\'s reference lines, its dots and its legend alike. The FC% column\'s own tooltip still claimed a hardcoded "green ≤30%, amber 31–38%, red >38%" until S720.',
        ...
        gotchas: [
          ...
          'The window is up to 24 periods (48 on "All periods"), so the 1000-row cap is reached faster here than anywhere else in IMS. opening_stock, closing_stock and staff_meals were the three reads the sales read\'s own paging comment did not cover, until S720.',
          'The period-count select had no overlapping-load guard until S720: arrowing 6 → 12 → 24 → All starts four concurrent loads, and a stale SMALLER result landing last left 18 of 24 rows showing dashes that read as "nothing happened in those months".',
          'The subtitle claimed "across all BS periods" whatever range was selected, and the printout and workbook named no range at all — a printed six-month sheet was indistinguishable from a two-year one, and the file was always PeriodComparison.xlsx.',
        ],
      ...
      },
      {
        ...
        formulas: [
          'Actual Net (per category) = Σ(line value net of the bill\'s allocated discount) − Σ(returns.qty×rate) for items in that category. The discount was left in until S720, so the figure a client checked their budget against was higher than Monthly Summary\'s Net Purchases for the same period — the page could report Over Budget on spend that was not over.',
        ...
        gotchas: [
          ...
          'A FAILED save reached only console.error until S720, under a banner promising "Budgets are saved automatically": the spinner cleared, the number stayed on screen, and the client had every reason to believe it landed until they came back next month and found it gone. It names the category now, says the figure is still on screen, and says not to reload before retrying.',
          'Spend on items with no category set fell out of the Actual column AND the Totals row, silently — so the page reported Under Budget on spend it had not counted and its total could not be reconciled against Monthly Summary. It is an "Uncategorised / unbudgetable" row now, included in Totals and excluded from the variance (there is no budget for it to be over or under).',
          'Blur fires whether or not anything was typed, so tabbing across untouched rows used to upsert amount 0 for each category it passed. Only a field that was actually edited saves.',
        ],
    ...
    sections: [
      {
        ...
        workflow: [
          'Select period. Filter by category/status (All / Low Stock / Out of Stock / In Stock / No activity). Each row shows a Source badge: Physical (a real closing count exists) vs Theor. (calculated, less reliable).',
        ],
        fields: [],
        formulas: [
          'Theoretical on-hand = max(0, Opening + Net Purchases − recipe-driven Usage − Wastage − Staff Meals). Usage = Σ sales × exploded recipe, over sales_entries run through selectDepletingSales (POS supersedes manual for the same recipe/day; pos_credit rows never restore stock) — the same rule Variance/Theoretical Variance/Shrinkage use. Arithmetic in src/modules/ims/stockcount/stockReportCalc.js (tested).',
          'On-hand = physical closing count if a closing_stock row exists (a row of 0 is a count), else the theoretical figure above. Stock Value = On-hand × per_uom_rate.',
          'Status: No activity if the item has no opening, purchases, usage, wastage, staff meals or closing row this period; else Out if on-hand ≤0; Low if par>0 and on-hand is strictly BELOW par (an item sitting exactly at par is OK — the same rule as Reorder Report and the dashboards); else OK. "Items Tracked" and "Out of Stock" exclude the no-activity items.',
        ],
        gotchas: [
          'Requisitions are NOT deducted (S695, decided with the owner). What the store issues is consumed by the recipes the kitchen then cooks, and sales × recipe already subtracts that; deducting both took every cooked-and-requisitioned item off twice, drove it negative, and the page blamed "a missing purchase entry". Requisitions.js\'s over-issue guard was changed the same way so the two agree on "available".',
          ...
          'A closing count of 0 used to be indistinguishable from "not counted" — Stock Count deleted the row for a 0 — so a counted-empty item fell back to the theoretical estimate and was valued into Total Stock Value as though on the shelf. Stock Count now stores the 0 (S695).',
        ],
      ...
      },
      {
        ...
        workflow: [
          ...
          'Both tabs: a Sort dropdown (per-tab keys, shared asc/desc toggle), a TOTAL footer row for the current filtered set (see the TOTAL row field for what it sums on each tab), and a Print button (printWithTitle, so "Save as PDF" gets a useful filename). Filters/tabs/buttons carry no-print so the printout is the header, stat cards and table only.',
        ...
        fields: [
          ...
          { label: 'TOTAL row', desc: 'Raw Items tab: sums the Value of the currently filtered rows. Sub-Recipes tab: deliberately does NOT sum the Value column. A row\'s Value includes the cost of any prep nested inside it, so adding the column up would pay for a nested sub-recipe twice — once on its parent\'s row and once on its own. The footer sums only the part each dish reaches directly (topValue, S721), so on a menu with nested prep the TOTAL is LOWER than adding up the column by hand, on purpose, and it counts each raw ingredient once. Qty is deliberately NOT summed on Raw Items (rows span different items in different UOMs) and only sums on Sub-Recipes when every visible row shares one yield UOM — otherwise it shows a dash, same rule as Purchases\' Qty total.' },
        ],
        formulas: [
      ...
      },
      {
        ...
        fields: [
          ...
          { label: 'Plates vs occasional', desc: 'A dish averaging ≥ 0.5 a day is shown as Math.ceil(average) plates (0.8 → 1, 2.8 → 3) — rounded UP because under-prepping a dish that sells most days costs more than one spare portion. Below 0.5 it is "occasional": listed with its average, not as a plate, but still included in the ingredient explosion at its average.' },
          { label: 'Forecast Revenue', desc: 'BEFORE VAT — gross less discount from POS bills, the same figure Owner Dashboard, Sales Entries and the Labor Forecast\'s learned sales-per-labour-hour mean by Revenue. It used to be the VAT-inclusive amount payable with VAT registration hard-coded true, which inflated a VAT-registered outlet\'s required hours ~13%. "≈" marks a priced estimate (forecast dishes × current ex-VAT selling_price) where the weekday has no POS bills.' },
          { label: 'Forecast Covers', desc: 'POS bills only; manual Sales Entries carry none. The column is hidden entirely for a client without Crest POS (keyed on clientModules.pos, the viewed client\'s subscription, not the session\'s posEnabled, which is true for every admin) with one sentence saying why — a column of permanent dashes was the previous state.' },
          { label: 'Ingredients to buy', desc: 'Σ over dishes of (horizon total forecast dishes × per-portion item qty), through explodeRecipeIngredients — the Reorder Report\'s walk: leaf items only, sub-recipes resolved through yield_qty, item yield_pct applied. Valued at per_uom_rate; items without a rate are counted but excluded from the total and said so. Demand only — it does not subtract stock on hand.' },
        ...
        formulas: [
          'Lookback window: 84 days, ending YESTERDAY — the day of the run is never a sample (a recompute at 8 AM used to feed one hour of trade into next week as a whole day). Capped at the 8 most-recent same-weekday samples per forecast day.',
          'Forecast Qty per dish = recency-weighted mean over those samples, weights 8,7,…,1 (most recent heaviest); a dish absent from a sample sold zero that day and weighs in as zero. Run log method: weekday_weighted_average (was weekday_moving_average, a plain mean).',
          ...
          'The 30-day horizon repeats the weekly pattern: days 8–30 use the same eight samples as days 1–7; only holiday multipliers differ.',
        ],
        gotchas: [
          ...
          'Manual sales-entry history only kicks in as a fallback/supplement once POS history covers fewer than 42 days — and only for the qty signal, never covers/revenue. A day present in both sources is one sample (the POS one), not two with the quantity split. The manual read matches source IS NULL as well as \'manual\' — the same predicate manual entry itself uses, since rows predating the column default read NULL — and is bounded to the periods the window can touch instead of every period the client ever had.',
          'New forecast rows are inserted before the old run is deleted (not delete-then-insert) — a failed recompute can\'t wipe out the last good forecast. The delete\'s error is checked: a refused delete used to leave both runs in place while the run log recorded a success, and the read-back then picked between them arbitrarily.',
          'sample_count / pos_sample_count need migration 20260908120000. Until it is applied, runForecast detects the missing column (PGRST204) and writes the forecast without them rather than failing; the page just omits the small print.',
          'The pure arithmetic is in src/utils/demandForecastMath.js (tested); demandForecastData.js is the Supabase orchestration around it. Keep new arithmetic in the pure file so it stays testable.',
        ],
        connections: 'Reads POS orders, Sales Entry, Recipe Costing (recipe_ingredients through explodeRecipeIngredients), Item Master and the Holiday Calendar (an HR admin page). The covers-level rows feed Roster\'s Labor Forecast tab (revenue → required hours). Demand, not stock: pair it with the Reorder Report or last stock count before ordering.',
      },
      {
        id: 'wastage-report',
        ...
        gotchas: [
          ...
          'This was the ONE page in the product that did not page its wastages read (fixed S719). The table is one row per item per DAY once Daily Wastage is used, and every other reader of it — Stock Count, Stock Report, Dead Stock, Reorder, FIFO, Stock Ageing — pages it. So past PostgREST’s silent 1000-row cap the report whose entire job is totalling wastage came back SHORT, in confident type, with no error to notice. A table’s own report is not automatically its most careful reader; it is usually the oldest.',
          'Value uses each item’s CURRENT per_uom_rate, not the rate paid when the stock was bought — the house convention for every stock valuation in IMS, and the Excel export now says so.',
        ],
      ...
      },
      {
        ...
        route: '/dead-stock',
        plan: 'Growth+',
        summary: 'Flags items with zero or very low consumption this period — capital tied up in idle inventory. Needs the month\'s closing count to say anything at all.',
        workflow: [
        ...
        fields: [],
        formulas: [
          'available = Opening + Purchased − Returned. used = Opening + Net Purchases − Wastage − Staff Meals − Closing (the shared computeUsed(), i.e. the same COGS definition every other IMS page uses).',
        ...
        gotchas: [
          'A closing count of 0 is a COUNT ("we looked and there was none"); no closing row at all is NOT. Until S717 the page treated the two identically, so every uncounted item computed as fully consumed, failed both tests, and dropped out — which meant an uncounted month rendered zero rows under the sentence "No dead or slow-moving stock this period." The most reassuring thing the page can say, said when it knew nothing.',
          'An item counted HIGHER than the stock available to it (usually a missing purchase bill) used to clamp to used=0 and therefore read as Dead — the loudest verdict on the page, produced by a data fault. Those are now excluded and counted in the same notice.',
          'Items with zero stock presence at all (available≤0 and no count) are skipped entirely — nothing to flag.',
      ...
      },
      {
        ...
        gotchas: [
          ...
          'Measurability is PER ITEM, not per period (S719). The page used to ask only whether the month had ANY closing rows, so on a month where 900 of 1000 items were counted the other 100 got a closing qty of zero, actual usage read as everything on hand plus everything bought, and each wore a full red Over flag with a fabricated NPR value — which was then summed into the headline "potential loss". Uncounted items are now excluded from every total, marked "not measurable" in the table and the export, and named in a banner.',
        ],
      ...
      },
      {
        ...
        plan: 'Pro',
        summary:
          'Tracks which purchased stock batches (lots with expiry dates) are expired or expiring soon, net of returns and consumption, using FIFO (oldest-batch-first) allocation. The most algorithmically involved report in the module.',
        workflow: [
        ...
        fields: [],
        formulas: [
          'Batches = every purchase in the window (net of returns booked against that purchase line), PLUS one undated carried-forward batch per item holding the window\'s opening stock.',
          'Consumption per item over the window = sold via exploded recipes (through the shared POS-supersedes-manual rule) + wastage + staff meals + returns that name no purchase line. Allocated against that item\'s batches oldest-first, carried-forward stock first of all.',
          'On Hand (per batch) = max(0, originalQty − returnedForThisBatch) − allocated consumption. Batches with none left, and batches with no expiry date, are not listed — the undated ones still absorb consumption first.',
          'This is the standard FIFO assumption, not a batch-precise trace: the schema has no batch-level consumption ledger, so nothing records which lot a portion actually came out of. Same basis as Stock Ageing, which shares the allocation code.',
        ],
        gotchas: [
          'This report is empty unless purchase entries are logged with expiry dates — the empty state says so explicitly.',
          'Three things changed in S717 and a client may remember the old behaviour. (1) Days Left was measured from TODAY whatever period was selected, so opening a month closed a while ago reported all of its stock as long expired, in red. (2) Only the selected month\'s purchases were read, so a batch bought last month and expiring next week was invisible — while that month\'s WHOLE consumption was still netted off its few batches, over-eating them. (3) Sales rows were summed raw instead of through the shared depletion rule, so a day sold in both POS and manual entry consumed its ingredients twice and a credit note put stock back.',
          'Batches with no expiry date are read and allocated against, but never shown. Reading only the dated ones made them swallow the undated ones\' usage too, so an item bought sometimes with and sometimes without a date under-reported its exposure.',
        ],
        connections: 'Shares its FIFO allocation with Stock Ageing (one function, stockAgeingCalc.js) and its consumption inputs with Variance and Stock Report. The expiry-risk leg of the "three risks" trio alongside Dead Stock (idle capital) and Reorder Report (understocking).',
      },
      {
        id: 'stock-ageing',
        title: 'Stock Ageing',
        ...
        fields: [
          ...
          { label: 'c/f badge and "Carried Into This FY"', desc: 'Stock that was already on hand when the fiscal year began (the first period\'s opening count). Its TRUE age is unknown and is at LEAST the figure shown, so it is aged from the window start and labelled rather than given a precision the data does not support. Because consumption eats oldest-first, ordinary turnover clears it before touching real purchases — so anything still sitting in it is genuinely stale. The card shows its VALUE: until S718 it summed the quantities instead and printed the result as "units", adding kilograms of flour to litres of oil to pieces of napkin, while the table\'s own TOTAL row printed a dash in the On Hand column for exactly that reason.' },
          { label: 'Oldest', desc: 'Age of the oldest batch of that item still on hand. Turns amber past 90 days.' },
          { label: 'As-of date', desc: 'The date every age on the page is measured to. It is the single most load-bearing fact here: an ageing schedule without one cannot be reconciled later, and the answer changes completely depending on it. Shown in the subtitle, the on-page note when it is not today, the print header and the workbook.' },
        ],
        formulas: [
          ...
          'Batch value = remaining qty × that batch\'s OWN purchase rate — not the current master rate. Ageing is about capital already committed, so it is valued at what was actually paid. The carried-forward batch is the ONE exception: it has no purchase line behind it, so it is valued at the current per_uom_rate — which moves every time a bill is entered. Three surfaces say so (the Stock On Hand tooltip, the c/f card, the row badge) because the tooltip used to claim the opposite for every batch on the page.',
        ...
        gotchas: [
          ...
          'Sales are deduplicated through the shared POS-supersedes-manual rule, applied PER PERIOD. Without the rule at all, a client running POS and manual bulk entry would over-consume its own batches and real stock would appear to vanish off the shelf. Applied across the whole year in one pass — which is what it did until S718 — it fails the other way: the rule keys on bs_day, a day number inside a month, so a POS sale on 5 Shrawan suppressed the manual sale of the same dish on 5 Bhadra, and a Bulk row (bs_day 0) was suppressed by a POS sale of that dish anywhere in the year. Suppressed rows are consumption that never gets subtracted, so stock that was eaten kept ageing on the shelf and the 90+ headline read HIGH.',
          'Early in a fiscal year the window is SHORTER than 90 days, so carried-in stock cannot reach the 90+ band however long it has really been sitting — and until S718 the headline therefore read NPR 0 with a green ✓ for the first three months of every year, which is the page saying "all clear" on the one question it cannot yet answer. It now renders the figure as a floor (≥, with △ instead of ✓) and the note names how much carried-in value is sitting in a younger band for that reason alone.',
          ...
          'A PAST fiscal year is aged to the end of that year, never to today. Until S594 it was aged to today unconditionally, which pushed every surviving batch of any past year into the 90+ band, turned the headline amber and reported the entire stock value as stale — wrong, and wrong in the direction that provokes action.',
          'The band columns are QUANTITIES and the TOTAL row under them is VALUE. They are different units in the same column, which is why every band header and the footnote both say so.',
        ],
      ...
      },
      {
        ...
        plan: 'Pro',
        summary: 'Conceptually a near-duplicate of Variance Report — theoretical vs actual consumption — but with its own locally-reimplemented ingredient-explosion logic. It flags at the same tolerance as Variance Report: the client\'s own Variance Flag % from Settings → Thresholds.',
        workflow: [
        ...
        formulas: [
          ...
          'Colour band: every figure goes through the shared varianceBand() (imsFormulas.js). First, a variance worth less than NPR 500 either way is "immaterial" — grey ≈, neither good nor bad, because it is rounding on a small line. Above that, within ±variance_flag_pct (the client\'s Settings → Thresholds value, default 10%) is green ✓; more used than the recipes explain is red ▲; less used is amber ▼. The Items Over Tolerance count and the Total Variance tile use the same band, so the tiles agree with the rows. Before varianceBand() existed this page hardcoded ±5% and ignored the client\'s setting, so the same item could be flagged here and not on Variance Report.',
        ],
        gotchas: [
          'This page and Variance Report are functionally near-duplicates with independently maintained recursion logic — a real maintenance risk. If Variance Report\'s shared util gets a bug fix, this page\'s local expandIngredients() copy does not automatically inherit it. The tolerance, by contrast, is now shared: both pages read variance_flag_pct. One small difference remains — this page also applies the NPR 500 materiality floor to each row, while Variance Report\'s per-row Over/Under badge uses the percentage alone — so a small-value item can read Over there and ≈ here.',
          ...
          'Measurability is PER ITEM (S719), the same fix Variance Report got in the same session and for the same reason: the page asked only whether the month had ANY closing rows, so an item missing from an otherwise-complete count read as “actual usage = everything on hand” and carried an Over verdict built out of an absence. Uncounted items now wear a “not counted” chip beside the name and are excluded from the totals and the Items Over Tolerance count.',
        ],
      ...
      },
      {
        ...
        gotchas: [
          ...
          'Requires at least one closed period. A client with none sees "No period has been closed yet…" and a pointer to close a period in Periods once its stock count is done. Until S747 that case left the page on its loading message forever.',
          'Its window spans SEVERAL periods, so every per-item-per-period read is multiplied by the period count — six periods × 200 items is 1,200 opening rows, over the silent 1000-row cap on a client far smaller than one that would trip a single-period page. All of them are paged as of S719; before that a truncated closing_stock read was indistinguishable from an uncounted item and produced fake shrinkage, which is the exact thing this report exists to measure.',
    ...
    sections: [
      {
        ...
        formulas: [
          'The page reads EVERY purchase line of the period and splits it with splitPurchaseVat() (purchaseTaxSplit.js), then shows the VAT half. It cannot read only the VAT lines: discount_amount is a BILL-level figure, so a bill\'s discount has to be apportioned across the whole bill before either half can claim its share.',
          'Each line\'s share of the bill discount = discount × (line value ÷ bill gross), via allocateBillDiscounts() — the same allocator Monthly Summary, Annual Summary, Period Comparison, Budget vs Actual and Consolidated P&L use. Taxable Base is the sum of the VAT lines\' post-discount values.',
          ...
          'A return is credited at the DISCOUNTED rate its original bill carried, not the list rate stored on vendor_returns.rate — so returning a whole discounted bill nets to exactly zero.',
        ],
        gotchas: [
          'Only purchases entered with the "VAT Incl. (13%)" toggle ticked are counted in the VAT figures — the non-VAT lines of the same bill are shown on the Non-VAT Report instead. The bill is still read whole, to split its discount.',
          'The "Non-VAT Purchases" stat card here is the SAME number the Non-VAT Report headlines — both read it off one split. Before 2026-09-10 this card showed gross non-VAT (no discount, no returns) while the other page showed net, so the two pages of one filing disagreed.',
          'Until 2026-09-10 a mixed bill\'s discount was counted twice across the two reports: this page took its proportional share and Non-VAT Report took the WHOLE bill discount, because its query filtered vat_inclusive=false and never saw these lines. A 1,000 discount on a 6,000-VAT/4,000-non-VAT bill was claimed as 1,600.',
          'The page explicitly says "For reference only — verify bills with your CA before filing."',
          'Excel exports carry a letterhead (company, PAN/VAT, address) and a scope line stating the period AND whether it is still open — figures from an open period can change after export.',
        ],
        connections: 'Its buildVendorSummary() helper lives in purchaseTaxSplit.js and is shared with Purchase 1L+ Report below. The two differ only in WHICH lines they pass it (VAT-only here, every line there) — the discount arithmetic is now identical, so vendor totals no longer drift between them for reasons nobody can see.',
      },
      {
        id: 'non-vat-report',
        ...
        fields: [],
        formulas: [
          'Total = Σ of each non-VAT line\'s post-discount value − non-VAT returns, where a line\'s discount share = bill discount × (line value ÷ bill gross). Same splitPurchaseVat() call as VAT Report, over the same rows — this page just reads the other half of the result.',
          '"Input VAT Credit: NIL" is a fixed, deliberate stat card contrasting this report against VAT Report.',
          'The Entries table footer shows GROSS TOTAL, then the discount and returns as their own lines, then NET TOTAL — so the Total column sums to its own footer.',
        ],
        gotchas: [
          'Until 2026-09-10 this page charged the WHOLE bill discount against the non-VAT half of a MIXED bill, because its query filtered vat_inclusive=false server-side and never saw the VAT lines. VAT Report simultaneously claimed its proportional share, so a 1,000 discount was claimed as 1,600 across the two halves of one filing, and "non-VAT purchases" differed between the two pages. Figures filed before that date read LOW here for any period containing a discounted mixed bill.',
          'Returns were not deducted here until 2026-07-27, while VAT Report had always deducted its own. Figures from before that date read high by the value of any non-VAT goods sent back.',
          'A return is counted as non-VAT by joining back to its purchase_entries.vat_inclusive — vendor_returns has no VAT flag of its own — and is credited at the discounted rate its bill carried, not the stored list rate.',
        ],
        connections: 'The other half of VAT Report\'s split, not a separate query: both pages call splitPurchaseVat() over every purchase line of the period, so the two halves provably sum back to the bill.',
      },
      {
        id: 'payment-summary',
        ...
        route: '/payments',
        plan: 'Starter+',
        summary: 'Breaks down purchase spend by payment method (Cash / Credit / FonePay) for a period, at BILL totals — net of discount and returns, including VAT where charged.',
        workflow: [
          'Defaults to the currently open period, falling back to the most recent period when none is open. Method Summary tab (aggregate) and Daily Breakdown tab (per-day, per-method).',
        ],
        fields: [],
        formulas: [
          'A bill\'s value is calcBillTotals().grandTotal — the SAME helper behind PurchaseBillForm\'s live total, the printed purchase voucher, Vendor Report\'s discount table and Outstanding Payables: (bill gross − discount) + 13% VAT on the VAT-inclusive portion net of its share of that discount.',
          'payment_method is a BILL-level choice written onto every line, so each bill belongs to exactly one method and is counted once however many lines it has (the "Bills" column). NULL reads as Cash.',
          'Per method: Net = Gross − Returns. A return is credited at its bill\'s discounted rate, plus VAT if the original line carried any, and inherits its linked purchase\'s payment method. % of Net Total = method.net ÷ grandNet × 100.',
        ],
        gotchas: [
          'Until 2026-09-10 this report had NO discount handling and NO VAT: "Gross" was literally Σ(qty×rate) per line, so it was neither the cost basis (net of discount) nor the money owed (incl. VAT) and tied to nothing — not the Purchases register, not Outstanding Payables, not the P&L. It now quotes the same bill total as all three.',
          'Until the same date, a client with no OPEN period selected nothing and still rendered a full report of NPR 0 across every method, with the total row reading 100%. It now falls back to the most recent period.',
          '"Credit" is what was BILLED to credit this period regardless of whether it has since been paid off — not the same concept as "outstanding". See Outstanding Payables for actual payment status; the two now quote the same per-bill figure, so a Credit bill\'s value agrees across both pages.',
        ],
        connections: 'Shares calcBillTotals() with Outstanding Payables (Credit bills becoming payables), the Purchases register and Vendor Report, so all four value a bill identically. billPayables() in purchaseTaxSplit.js is the shared entry point.',
      },
      {
        ...
        formulas: [
          'Bill Total = what the vendor actually invoiced, via the SAME calcBillTotals() helper that backs the live total in PurchaseBillForm and the printed purchase voucher: line values net of any goods returned against them, minus the bill-level discount (taken ONCE per bill, however many lines repeat it), plus 13% VAT on the VAT-inclusive portion net of its share of that discount.',
        ...
        gotchas: [
          ...
          'Until S747 an OLDER multi-line bill (one saved before purchase_group_id existed, grouped by the vendor + invoice + day fallback) had its discount taken off once per LINE instead of once per bill — a five-line bill\'s discount came off five times, so its Bill Total and remaining balance read too low. Any such bill part-paid or settled before then is worth re-checking against its printed voucher.',
        ],
    ...
    sections: [
      {
        ...
        fields: [],
        formulas: [
          'Revenue per recipe = Σ(qty × price − that sale\'s discount), using each sale\'s own snapshotted unit_price where available rather than a single blended current price, and subtracting sales_entries.discount on every row.',
      ...
      },
      {
        ...
        gotchas: [
          ...
          'Trend and Change % compare the last two PRICED purchases, so a free line (rate 0, valid since S698) no longer shows as a 100% price drop or switches off the >5% master-rate warning. Stable is a percentage band, not an absolute NPR 0.01 — that absolute swallowed a 20% rise on a NPR 0.05/GM staple while catching everything on expensive items (S728).',
          'The header says what the rising prices COST — rupees on the latest delivery of each item — not only how many items are rising.',
      ...
      },
      {
        ...
        fields: [
          { label: 'Net Spend', desc: 'Gross − Discount − Returns, all ex-VAT. Payment Summary also takes the discount off, but values each bill at calcBillTotals().grandTotal — the amount owed, INCLUDING 13% VAT on the VAT lines. So when the two pages disagree for a vendor, the gap is mostly VAT, not discount.' },
        ],
        formulas: [
          ...
          'Bill-level payment status reuses the same aging buckets as Outstanding Payables (Current/31-60/61-90/90+). That aging function (billAging) is the one piece still kept as a local copy in this file; the money arithmetic is all shared — allocateBillDiscounts, netFactors, returnBase and calcBillTotals are imported, not copied.',
        ],
        gotchas: [
          ...
          'The Daily Breakdown matrix now has a column for every vendor with purchases OR returns, plus an Unassigned column, and its Day Net Total follows the columns shown — the totals used to cover the whole period while the columns covered only vendors with purchases, so the rows could not be added across (S728).',
          'The Bills column counts BILLS, matching the drill-down. It counted line items under a "Transactions" header, so a 12-bill vendor with 8 lines a bill read as 96.',
          'The drill-down values a bill the way Outstanding Payables does — returns netted, discount applied, VAT on the taxable part — so a settled bill no longer reads Partial with a phantom balance. The Payable column shows the figure the status is measured against (S727).',
        ...
        connections: 'Overlaps with Outstanding Payables (bill valuation and status — same calcBillTotals, local aging buckets) and VAT/Non-VAT Report (the discount factors and return pricing come from the same purchaseTaxSplit.js helpers) — a good report to present first when introducing vendor analytics, with Payment Summary, Outstanding Payables, and VAT/Non-VAT as narrower cuts of the same underlying data.',
      },
      {
        ...
        workflow: [
          ...
          '🖨 Print and Export Excel output the summary table as shown. The workbook carries a second "Ingredient Detail" sheet with every traced line, since the on-screen panel only previews the top 12 (use "Show all" there to see the rest).',
        ],
        fields: [
          { label: 'Cost of Sales vs Net Purchases (and Reliance Gap)', desc: 'Cost of Sales is the supplier\'s share of the ingredient cost behind what sold; Net Purchases is what you actually spent with them (same figure Vendor Report calls Net Spend). Reliance Gap is the difference between the two shares in percentage points — a large positive number means you depend on that supplier more than the purchase ledger suggests. It was headed "Δ" until S594; a bare Greek letter is the wrong register for an owner scanning between rushes.' },
        ...
        gotchas: [
          ...
          'Top Supplier Share is measured against NAMED suppliers only, so a month largely fed from earlier stock cannot make your concentration risk look safer than it is, and "No vendor recorded" can no longer be named as your largest supplier (S728). Reliance Gap compares like with like for the same reason.',
          'The Ingredient Detail sheet carries the SHARE of an item consumed by each supplier. It used to repeat the whole-period quantity on every supplier row, so the Qty column totalled to a multiple of what was actually consumed.',
    ...
    sections: [
      {
        ...
        workflow: [
          ...
          'The Owner login and any platform admin both resolve to Manager automatically — they never need an ims_role set. Note the Owner test is negative: an account counts as Owner only when it has none of pos_role, hr_self_service, ims_role or hr_role set, so giving someone an ims_role (or any other staff role) deliberately demotes them out of Owner-level access.',
          '"+ Add Staff" has up to four modes. HR Employee (only when the client has HR) links the new login to an existing hr_employees record so the name never drifts between modules. Existing User (Owner/admin only, and only when there is an eligible login — see below) assigns a role to a login that already exists rather than creating anything. IMS-only Staff creates a fresh email + password login. Count PIN (always offered, S737) creates a name + 4–6 digit PIN account with no email or password, fixed at Staff rank, that signs in at /ims/count and reaches Stock Count only.',
          'Reset Password sets a new password immediately — there is no email or reset-link flow, so the new password has to be handed to the person directly. A Count PIN account has no password, so its row shows Reset PIN instead.',
        ],
        fields: [
          ...
          { label: 'Supervisor', desc: 'Everything Staff has, plus Periods, Item Master, Vendors, Purchase Orders, Recipe Costing, and the Stock and Summary reports (Monthly/Annual Summary, Period Comparison, Budget vs Actual, Stock Report, Reorder, Stock Movements, Wastage, Dead Stock, Variance, FIFO, Stock Ageing, Theoretical Variance, Shrinkage) — plus the two Suite-billed, IMS-shaped pages, Demand Forecast and Fixed Assets. Item Master and Vendors sit at this tier specifically because they expose purchase rates — Staff can still log purchases normally, since the item/vendor picker inside Purchases does not require visiting those pages.' },
          { label: 'Manager', desc: 'Everything above, plus this page, Settings, Overheads, Menu Pricing, Menu Engineering, Menu Repricing, every Finance report (VAT, Non-VAT, Payment Summary, Outstanding Payables, Purchase 1L+, Vendor Balance Confirmation), and every Menu & Vendor report (Best & Worst Sellers, Recipe Margin, Combo Builder, Price Tracker, Vendor Report, Supplier Contribution). The dividing line is money: this tier is what exposes margin, payables, and supplier pricing. It is NOT Owner-equivalent: a Manager may create logins and move staff and supervisors between roles, but only the Owner or an admin can change, delete or reset the password of another MANAGER, and a Manager cannot change or delete their own row (the page shows "(you)" and disables those controls). Both are enforced in admin-user-ops (requireManageableTarget, S729), not only in the page.' },
          { label: '"Existing User" mode', desc: 'Assigns an IMS role to a login the client already has (e.g. one created through Admin → Clients → Manage → Users) rather than creating a second account for the same person. Picks from the get_ims_eligible_users RPC and calls update_ims_role — no new login, and no email/password fields are shown. Owner-only: an IMS Manager will not see this mode at all, because the accounts it offers are by definition non-staff logins, and converting one into staff is a decision about who owns the account rather than about who manages the module. The server also refuses to convert a client\'s LAST plain login (S729): the Owner is the absence of staff markers, so converting the only one would leave the client with no Owner at all.' },
          { label: 'Manage Roles (custom role names)', desc: 'settings.ims_custom_roles. The scheme is "custom roles if any, else the three defaults", which until S729 meant the FIRST custom role added silently replaced Staff / Supervisor / Manager in every dropdown while every existing login still carried one of those titles — each row\'s select had no matching option and rendered blank beside a badge still reading Supervisor. Now the first custom role is seeded alongside the defaults, a role that logins still hold cannot be removed (the holders are named), Reset to defaults refuses while anyone holds a custom title, and a login whose title has somehow fallen out of the scheme renders its stored title as its own option with an amber "orphan" mark rather than a blank. Changing a role\'s level moves every login holding it, in parallel, and names any that could not be moved — a peer Manager\'s row is refused for a Manager caller, and that refusal is now visible instead of silently leaving one badge behind.' },
        ],
        formulas: [],
        gotchas: [
          ...
          'The list RPC itself was the other half of the 2026-08-10 finding, and it stood open until S729: get_ims_staff_list (and get_hr_role_staff_list) gated on same-client alone, no rank — so a POS PIN waiter or an HR Self-Service employee could call it with their own client_id and read every IMS login\'s real sign-in email. Both now require admin, the Owner, or a Manager of that module (migration 20260910120000). The page redirects an under-ranked visitor, but its useEffect had already fired the RPC — a page guard is a guard on the page, never on the data.',
          'Passwords set here — on creation and on Reset Password — take the product\'s one policy (MIN_PASSWORD_LENGTH plus weakPasswordReason from weakPasswords.js), the same check Signup and Reset Password apply. Until S729 this page hardcoded "8 characters" and nothing else, so "password" and "12345678" were accepted for a real /login credential. The server additionally screens creation against Have I Been Pwned.',
          'Every write on this page goes through the admin-user-ops Edge Function with the service role, so the audit_logs row it produces on profiles has NO actor — log_audit() reads auth.uid(), which is null under the service role. A role grant, a revoke or a password reset is recorded as having happened but not by whom. Known and unfixed as of S729; it applies equally to POS Staff and HR Staff.',
    ...
    sections: [
      {
        ...
        summary: 'Seven tabs configure the client-side of Settings: Thresholds, Item / Vendor / Sub-Recipe / Product Codes, Recipe Categories and Theme (Sub-Recipe Codes and Recipe Categories only with recipe_costing, i.e. Growth+). None appears in the admin\'s own Settings view, which shows Branding / Property / Support / Plan Pricing / Data / Theme / Guides — and only the first three plus the Support tab\'s lower card are about the client being viewed: Plan Pricing is the platform\'s one price list, Theme is the browser, Guides is reference prose, and Data links to the tools in Admin → Clients. The page-level Save Changes writes ONLY the columns of the tab you are on (TAB_FIELDS in Settings.js), as a diff against the loaded row — never the whole row, and since S747 never another tab\'s unsaved edits either: an edit left on one tab waits for that tab\'s own Save. That matters because `settings` is one row per client that nine other pages write their own columns onto (POS discount reasons, note presets, ticket routing, reservation settings, TADA rates, the three custom-role schemes, the combo discount): until S730 the button sent the whole row as it stood when Settings LOADED, so an owner saving a threshold put back whatever a manager had changed on the till meanwhile, and both saves reported success.',
        workflow: [],
        fields: [
          { label: 'Item Codes', desc: 'Sets item_code_prefix (default ITM). New items get the next sequential number automatically. "Regenerate All" renumbers every REAL item alphabetically from {prefix}-001 — used to close gaps after deletions. Sub-recipe mirror rows in `items` are skipped: they carry the sub-recipe\'s own SRC code, which the ingredient rows print, and until S730 the renumber stamped ITM codes over them. Surfaces on Price Tracker, purchase entries, stock sheets, audit trails.' },
          { label: 'Vendor Codes', desc: 'Same pattern for vendor_code_prefix (default VND), archived vendors included — vendor_code has no unique index, so leaving an archived vendor on a code an active one is renumbered onto would give two suppliers one code the day it is restored. Shown as a badge in Vendor Report and used as a secondary search field there.' },
          { label: 'Sub-Recipe Codes', desc: 'Same pattern for sub_recipe_code_prefix (default SRC), scoped to recipes where category = "Sub-Recipe." Gated behind the recipe_costing feature flag. The renumber runs in TWO passes because recipe_code is unique per client (recipes_client_recipe_code_key): a one-pass renumber gives row A the code row B still holds and the index refuses it — and until S730 every per-row error was dropped, so that refusal was reported as "✓ Renumbered". Each sub-recipe\'s mirror item takes the same code in the same run; nothing else ever updates it.' },
          ...
          { label: 'Recipe Categories', desc: 'Free-form list feeding the recipe-form category dropdown and the category filter tabs on Best Sellers, Recipe Margin, and Menu Repricing. "Sub-Recipe / Prep Item" is a protected, system-managed category that can\'t be added/removed here — it\'s the sentinel value every cost report explicitly excludes. Each row shows how many recipes hold it (a paged `recipes` read), and removing one in use confirms with the count first. Removing a category does NOT retag existing recipes: they keep the label, keep their own tab in Recipe Costing, and the recipe form now offers the orphaned category as its own option marked "(no longer in Settings)" — before S730 the select silently DISPLAYED the first category while the form still held the old one. Orphaned categories are listed under the table with an "Add back" button. Saves one column (`recipe_categories`), on its own button; the page-level Save does not render on this tab.' },
          { label: 'Thresholds', desc: 'fc_warning_pct (35) / fc_critical_pct (45) colour EVERY food-cost figure in the product: the Dashboard card, Monthly Summary, Recipe Costing\'s filter pills AND its FC% column, Menu Pricing, Menu Repricing, Recipe Margin, Annual Summary and Period Comparison. Those pages each carried their own hardcoded 30/38 scale until 2026-08-13. expiry_warning_days (7) sets FIFO/Expiry Report\'s "expiring soon" window. variance_flag_pct (10) is both variance reports\' flag threshold (S659). Every reader is `parseFloat(x) || default`, so a stored 0 IS the default — the page therefore refuses 0 and negative values, refuses a critical level at or below the warning level, stores a cleared box as NULL (Postgres refuses \'\' for numeric), and shows the default as the placeholder. Until S730 none of that was checked: 0 saved, \'\' failed with a type error, and critical ≤ warning saved and made the amber band unreachable.' },
          { label: 'Theme', desc: 'Per device, in the browser\'s local storage — nothing is written to the account. Two presets plus "Follow device" (the provider\'s SYSTEM_KEY, the Crest Staff app\'s default), which this tab could not select until S730: a device that already had it saved showed no card marked.' },
        ],
        formulas: [],
        gotchas: [
          'Regenerating codes (Item/Vendor/Sub-Recipe) is a bulk renumber — always confirms before running since it changes every existing code, not just new ones going forward. Product Codes is the exception: it fills gaps only and leaves existing codes alone. All four now check every row\'s write and stop at the first failure, saying how far they got; before S730 the three renumbers dropped every per-row result and reported success whatever happened.',
          'Pressing a Renumber button saves that tab\'s prefix and nothing else. It used to save the entire form, i.e. every other tab\'s unsaved edits, as a side effect.',
          'An edit typed on one tab survives a save made on another: Save Categories and the Renumber buttons both re-read the row, and until S730 the re-read reseeded the whole form and wiped whatever was pending on Thresholds. A switch of client in the admin top bar still replaces everything — an edit is never carried across tenants.',
          ...
          'The page requires the IMS Manager rank, but the `settings` row accepts an UPDATE from any authenticated account of the client at the RLS level (migration 20260707150000 — POS staff need it for Table Management\'s tabs). A rank check on this page is a check on the page.',
          'The write scope is the current tab\'s own columns (TAB_FIELDS), not a fixed list, and that is what keeps the threshold validation self-limiting: the check only runs when a threshold is part of the save, so a client row left holding a 0 or a critical level below its warning level — both saveable before S730 — cannot refuse an admin\'s branding or consultant save. It used to, and then sent them to setActiveTab(\'Thresholds\'): a tab absent from ADMIN_TABS, so no tab rendered as selected and the panel appeared with nothing above it. Until S747 the button wrote the union over every visible tab, so Save on Branding also committed a consultant number half-typed on the Support tab.',
          'A settings row that could not be READ blocks editing rather than showing defaults (S747). If a client\'s settings fail to load, every tab that edits that row (Branding, Property, Thresholds, the three code tabs, Recipe Categories) shows the error and "nothing on this tab can be edited", with no Save button. Plan Pricing and the Support tab\'s Crest contact card do the same when Crest\'s own platform row fails to load. Before, the page quietly filled the boxes with the built-in defaults ("Crest Suite" as a client\'s brand, a blank VAT number, the shipped plan prices), and pressing Save would have written those over the real values.',
          'On the admin side, Invoice Prefix and VAT Registered both reach bills ALREADY ISSUED. `pos_orders` stores only invoice_no and invoice_fy; the printed number and the TI/PB bill type are assembled when a bill is printed, from the settings row current at that moment. So changing the prefix re-numbers every past invoice and switching VAT off reprints past tax invoices as PAN bills. Both now ask first and name the consequence — and since S747 so does setting a FIRST code, because bills issued with no code (TI2238-82/83) reprint with it added (TI2238-CAC-82/83).',
          'The Invoice Prefix is never filled in for you (S747). The box stays blank, with the placeholder "Your Business Code", until someone types a code; left blank, bills print without one. It used to be auto-filled from the property name ("Casa Acai Cafe" → CAC), and because that made the form differ from the saved row, the next Save on the tab committed it with no warning — silently renumbering every past bill.',
          'Admin → Clients → Manage → Settings edits the same columns as the admin Branding / Property / Support tabs here, including the three consultant fields (it called them "Upgrade Contact" until the names were unified). Same data, two screens — last writer wins between them.',
        ],
