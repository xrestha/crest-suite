    ...
    sections: [
      {
        ...
        workflow: [
          'Loads the client\'s open monthly_periods row automatically. If none exists, an admin gets a banner linking to Periods; a client\'s own login is told Crest opens the first month and each next one opens when the current one closes, with the support contact (S790 — only admin can create a month from nothing).',
          'S790: a new client (an approved trial, or the first 60 days after approval) sees the setup guide at the top — "Getting started with Crest", a short checklist per module the client has (Stock & costing, Till & bills, Staff & payroll), for the Owner and email-login managers only. A box ticks when the thing really exists (the first item, the first bill), never because a page was opened; any step can be skipped; "Hide for now" shrinks it to one line; each person\'s choices are saved on their login (onboarding_progress). An admin viewing the client sees it read-only. It is always in Help → Getting Started, where an established client can start it too ("Start the setup guide"; also "Setup guide" in the account menu) — it then stays on their dashboard until finished or put away.',
        ...
        fields: [
          ...
          { label: 'Target dotted line', desc: 'What a usual day looks like, learned from the 28 days before the open month began (4 of each weekday, read from the previous one or two periods). Both are an average per weekday, each over its own week. Sales: over the days with a sales entry, so a Saturday target sits above a Monday one. Purchases: over every day in the window, a day with no bill counting as zero, so the restock day sits high and a no-buying weekday sits at zero. The weekly total is the same as a flat daily average of that window; only its shape across the week differs (S783; S780 shipped it flat). Captured ONCE per period and never moved, which is what makes it a target. Because the window ends at Day 1, it is the same whichever day it is captured on. A client with under 14 days of history instead gets one from its own month after 7 days, and the Tip says so. Its own hue per metric (blue for sales, orange for purchases). Before S780 this was a straight line fitted through the month\'s first 5 days, which ran to zero mid-month whenever those days happened to slope down. Older snapshots (that slope, and S780\'s flat purchase line) are replaced once, automatically; each metric carries its own snapshot model number, so replacing one never touches the other.' },
          { label: 'Forecast dashed line', desc: 'LIVE, recalculated on every load. Each remaining day = the Target\'s value for that weekday × a pace factor: 1 + w × (actual so far ÷ what the Target expected for those days − 1), with w = days so far ÷ (days so far + 3). About half this month\'s pace after 3 days and 70% after 7, so a real change in trade shows within a week. With no Target yet, it is this month\'s plain daily average, from Day 3. It has no slope and no ceiling, so it can neither run to zero nor stick at a cap. If the history read fails, no Target is captured (one failed read must not lock a one-week Target for the whole month), and the footer says so.' },
          { label: 'Tooltip arrows on the actual rows', desc: 'Hovering a day shows how far it ran off its Target. The ARROW is literal — ▲ above the target line, ▼ below — because it has to agree with the line you can see. The COLOUR is the verdict, and the two metrics have opposite polarity: for Sales, above target is green; for Purchases the reverse, because spending under your usual pace is the win. The percentage beside it is the gap as a share of that day\'s target. A grey ≈ replaces the arrow when the day landed within tolerance of target — 2% of it or NPR 100, whichever is more forgiving — because without a dead zone every single day earns a verdict, and a chart lit green and red on all thirty says nothing at all. Two thresholds rather than one: 2% of a big delivery day is fairly ignored, but on a quiet Sunday even a 10% swing can be NPR 20.' },
          { label: 'Weather strip (Growth+, S785)', desc: 'Top right of the Dashboard header, for any client with the weather (hasWeather: the weather_forecast Growth key, or the POS or HR module, S786) and a city set, not only IMS clients: today and the next 3 days, each as a picture, the high / low air temperature and mean cloud over 05:45–23:45 (the same window as the rain), and a word from weatherLook() — Light rain from 1 mm, Rain from 5 mm (exactly where the forecast adjusts, so a tagged day never reads Light rain), Heavy rain from 20 mm, Storms when a forecast block overlapping the window names thunder, otherwise Sunny / Partly cloudy / Cloudy by cloud cover. Each day is two lines beside its picture (day + high/low, then the word + a tag), so the strip stands about as tall as the title; on a phone the days go 2 × 2. A day the chart\'s forecast moved carries a "×N%" tag — the same days the chart footer names — explained once under the strip ("×60% = rainy-day sales forecast") and in full in its Tip, which says lowering or raising by the Owner\'s figure (30–150). The tags show only to a login that sees the sales chart (IMS staff and up). Each day\'s Tip gives the BS date, the mm, and for today either that the figure covers only the rest of the day or which parts are frozen. A day already stored whole keeps its rain and high/low from the forecast made before it began (what the sales forecast uses); a later fetch that day refreshes only its sky and thunder, and fills a missing high/low. Rows written before S785 carry rain only until a fetch rewrites or refreshes them: the temperature shows a dash and a dry day reads Dry rather than guessing the sky. "Today" is the viewer\'s Nepal date, and the weather is re-asked each time Dashboard is opened. The MET Norway credit and the fetch time sit underneath; a failed reply, or one with no row for any of the four days, says "Weather unavailable right now". With no city, the Owner or any module\'s manager gets a Set your city button, and with one a Change city button under the days; both open WeatherCityPicker in a small box, the same picker as POS Setup → Weather. The HR Dashboard carries the same strip (WeatherHeaderSlot + useWeatherStrip, S786) without the tags.' },
          { label: 'Rain adjustment (Growth+, S784)', desc: 'The dashed SALES forecast only. The Owner sets the outlet\'s city and rain_sales_pct ("a rainy day sells about N% of a normal day", 30–150, blank = off) in Settings → Weather. The weather-forecast Edge Function reads MET Norway\'s forecast for that city (cached 3 h per location in weather_daily, service-role only) and returns rain per Nepal day, summed 05:45–23:45 only. On a rainy day (≥ 5 mm, a COMPLETE day\'s total, i.e. forecast before the day began) within the next 7 days — and within 7 days of when the forecast was fetched, so a stale forecast steers less — that day\'s forecast × the percentage; past rainy days of the month get the same factor inside the pace calculation, so a wet week does not also drag the pace down. The pace feeds every remaining day, so days beyond 7 are not scaled themselves but can still move. Purchases and the frozen Targets are untouched. The footer names the adjusted days (and says "no rain forecast" only for days the forecast actually covers, next month\'s included), and shows the outlet\'s own measured rainy-vs-dry figure once there are 5 rainy and 5 dry recorded days — shown, never applied. Weather is only recorded from the day a city is set, and a monsoon-free month will rarely produce 5 rainy days.' },
        ],
        formulas: [
      ...
      },
      {
        ...
        workflow: [
          'Client view: a table of the client\'s own periods, newest first. The "+ New Period" button (pick BS Year + BS Month) and the Close / Reopen actions on an open row are shown to ADMIN only; since S756 the Owner\'s CLOSED rows also carry "Add missing bills" and "Resync Opening Stock", because the Owner edits a closed month in place — a client\'s own login sees the list, "Show Archived" and (with POS) "Post POS bills to Inventory", and closes its month from the Dashboard\'s "End <month> & Start <next>" banner once the month has ended. Since S738 a period created by hand carries the previous EXISTING period\'s closing count into its opening stock, exactly as Close & Start Next does — and the notice says which month it came from, or that there was nothing to carry.',
      ...
      },
      {
        ...
        gotchas: [
          ...
          '"Used In" covers every table that records an item — recipes, customization options, purchases, opening/closing stock, wastage, staff meals, requisitions, vendor returns, par levels, purchase orders and stock movements — and shows a code for ANY record there, whatever its quantity. A Closing Stock count of 0 shows CS, because it blocks a delete exactly as a real count does (S766: the column used to skip zero-quantity rows, so an item could show "—" and still refuse to delete). To retire an item whose only record is a zero count in the open month, blank its cell on Stock Count first — a blank cell removes the row, a 0 keeps it. The Unused filter therefore lists exactly the items that can be deleted. If any of those cannot be read the column says "not checked" instead of showing a blank, and deleting is blocked: three of them would be deleted along with the item rather than stopping the delete, so an unchecked item is never safe to remove.',
        ],
      ...
      },
      {
        ...
        workflow: [
          ...
          'Keying a bill off paper is a keyboard job (S765): "+ Add Item" now puts the cursor straight into the new line\'s item box, and pressing Enter in the Days field at the end of a row starts the next line and focuses it — so a 15-line bill no longer means tabbing past every field of every line already entered just to reach the Add button.',
          'An unsaved bill now survives the page dying (S779). What is on screen is mirrored into this browser as it is typed — debounced while typing, written immediately when the tab is hidden — so a Chrome auto-update restart, a tablet discarding the backgrounded tab while the user is in another app, Chrome\'s memory saver, or a deploy landing mid-entry no longer costs twenty minutes of keying. Reopening the screen restores it behind an amber "Brought back what you were typing" bar carrying the time it was last on screen and a button to start clean (or, on an edit, to drop the unsaved corrections). It never reaches the server, is keyed to the login that typed it as well as to the bill (a shared store-room tablet must not hand the next PIN what someone else typed — the posLockedCart.js rule), is only ever the DIFFERENCE from the bill as opened, and is cleared the moment Save lands or Cancel is pressed — so anything restored is by definition still unsaved. Line table order is Item | Qty | Rate | Total | VAT | Amount: the tick moved out from between Rate and Total (S779, owner\'s call), which had split the two boxes holding the same fact and put a checkbox in the tab path between them.',
      ...
      },
      {
        ...
        workflow: [
          ...
          'On a TOUCH device the count screen is a list of per-item cards with a big right-aligned number box, a progress bar and a fixed Save bar at the bottom — not the desktop table. Which screen you get is decided by whether a FINGER is the main pointer, not by screen size (S765): every phone and every tablet gets the cards, and a laptop with a touchscreen that you drive with a mouse correctly keeps the table. Until S765 the test was screen width under 768px, which is exactly iPad-portrait width — so no tablet had ever reached the screen built for it, and a store-keeper counting a shelf on an iPad got the desktop table with boxes too small for a thumb.',
          'Each card says where its number stands, in words: "Not saved yet" (typed but not stored), "Saving…", "✓ Saved" (the server has it) or "Saved on this device — waiting for a connection" (queued offline). Before S765 the card lit up the moment you typed a digit, so a figure sitting unsaved looked exactly like one that had landed — which matters most on a shared tablet where two people are counting at once.',
          '"Clear All" is deliberately absent from the touch screen (owner decision, S765): it wipes every quantity on the tab, and a button that does that sitting under a thumb next to Save is an accident waiting to happen. Managers can still use it from a desktop. "↩ Last month" and "Save All" are both in the bottom bar.',
          ...
          'What a PIN count account sees (S761): the Closing Stock tab and nothing else, with no NPR value per line and no running total. A phone was showing all seven tabs across three wrapped rows before a single item appeared, the DEFAULT one was Opening Stock (an entry grid with its own Save All, i.e. the month\'s starting basis one tap from being rewritten), and Summary put the outlet\'s COGS and an Excel export of its cost base on a shared store-room handset. Anyone who signs in with an email — including a store keeper at Staff rank — still gets the whole page; only the PIN accounts are trimmed.',
        ],
        fields: [
          ...
          { label: 'Value (NPR) column', desc: 'Qty × per-unit rate, with a running total under it. Shown to the Owner, supervisors and managers; hidden from every Staff-rank counter since S761, on the entry grids of all four tabs and whether or not blind counting is on. Counting an item needs its name, its unit and a box to type into — what the shelf is worth is the owner\'s business, and on a phone the column was taking width from the thing being counted. Separate from blind count, which hides reference QUANTITIES and only while it is switched on.' },
          ...
          { label: 'Blind count', desc: 'Settings tab. Takes the Purchased, Returned and Value columns off the Closing Stock tab for a staff-level counter, so they write what is on the shelf instead of confirming what the system expected. Since S761 it also takes System Ref Qty off the Print Sheet and closes the Summary tab for that counter (with a note saying why) — those two carried the same figures, so until then a blinded counter could read them in two taps, and on paper. This changes what is SHOWN, not what the browser can reach — it is counting discipline, not a lock, and the tab says so.' },
          ...
          { label: 'Count PIN accounts', desc: 'Created from IMS → IMS Staff → Count PIN: name plus a 4–6 digit PIN, no email and no password. They sign in at /ims/count by tapping their name, and reach Stock Count and nothing else. Since S761 they get ONE tab inside it — Closing Stock — with no NPR value beside each line: Opening Stock, Wastage, Staff Meals, Summary and Print Sheet are not on their bar at all. Five wrong tries locks the account for 15 minutes; a manager can reset the PIN from the same screen.' },
        ],
        formulas: [
    ...
    sections: [
      {
        ...
        workflow: [
          ...
          'Build-your-own (S760, needs Crest Customization): tick Build-your-own under Category and the dish may be saved with NO ingredients — its plate is the guest\'s choices, whose stock lines live on the options in Custom → Option Groups. Saving a newly marked dish offers "Set up choices →". A Build-your-own tab appears beside the category tabs, and each such row shows a cost RANGE with a disclosure that opens a table per size.',
        ...
        fields: [
          ...
          { label: 'Build-your-own', desc: 'A mark, not a category — the dish keeps Food, Beverage or whatever it is, so ticket routing, its product code and the category splits are unchanged. Shown only when Crest Customization is live. It makes the till always open the dish\'s choices, makes the QR menu walk them step by step, and swaps this page\'s single food-cost figure for a range. A marked dish is also allowed to have no ingredient rows, and its "No BOM" flag is suppressed, because its stock comes from the choices.' },
        ...
        formulas: [
          ...
          'Build-your-own range (S760) = for EACH size: the cheapest build (the required picks at their lowest price, nothing optional) and the typical build (the most-picked selection over the last 30 days once 10 plates exist, otherwise the dish\'s pre-selected choices — the row says which), each priced and costed with the size\'s Portion applied. The cell shows the lowest and highest of those costs; the FC % cell shows the band of the DEAREST build, so the mark warns on the worst case.',
        ...
        gotchas: [
          'A build-your-own dish whose choices or item rates cannot be read shows "not checked", never the fixed-ingredient figure — on a dish with two ingredients that figure would read as a near-zero food cost, which is the most flattering possible rendering of "we do not know". A marked dish with no groups reads "no choices yet".',
        ...
        connections: 'Build-your-own dishes read their choices, stock lines and item rates through Crest Customization (Option Groups) and 30 days of POS lines for the typical build. Feeds Sales Entry\'s recipe list, Menu Pricing, Menu Engineering, Menu Repricing, Best Sellers, Recipe Margin, Dashboard\'s Menu Health, Overheads\' allocation panel, and Requisitions\' on-hand estimate.',
      },
      {
        id: 'menu-pricing',
        title: 'Menu Pricing',
        ...
        workflow: [
          ...
          'IMS branch, build-your-own dishes (S760): the row wears a Build-your-own badge, Food Cost and FC % show the RANGE across sizes instead of one figure, and the disclosure opens the per-size table. New FC % stays blank for such a dish — a single projected percentage off its fixed ingredients would be the near-zero figure the range exists to replace; the range follows the new price once it is saved. The POS-only branch shows none of this, because it has no costs at all.',
    ...
    sections: [
      {
        ...
        fields: [
          ...
          { label: 'Weather (Owner, Growth+, S784)', desc: 'weather_city / weather_lat / weather_lon (written together by a city picker — the app\'s Permissions-Policy blocks device geolocation) and rain_sales_pct. Only the Owner sees the tab (an IMS manager does not), whenever the client has the weather; the rainy-day box shows only with the weather_forecast Growth key. It saves only these four columns. The city is also set from POS Setup → Weather and the dashboards\' box, all through cityPatch; the database lets the Owner, admin or any module\'s manager change the city, and only the Owner or admin the rainy-day figure (settings_guard_staff_roles, 20260923130000). The coordinates are what the weather-forecast Edge Function reads, from the row and never from the request. rain_sales_pct is refused outside 30–150 and at 0; blank is stored as NULL and means no adjustment. rain_sales_pct is consumed only by the Dashboard\'s Daily Purchases vs Sales sales forecast; the city also drives the Dashboard header\'s weather strip (S785), for every client with the weather, whichever modules it runs.' },
        ],
        formulas: [],
        gotchas: [
          ...
          'The page requires the IMS Manager rank, but the `settings` row accepts an UPDATE from any authenticated account of the client at the RLS level (migration 20260707150000 — POS staff need it for POS Setup\'s tabs). A rank check on this page is a check on the page.',
