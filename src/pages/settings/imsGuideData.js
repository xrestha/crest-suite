    ...
    sections: [
      {
        ...
        gotchas: [
          'Different reports compute "food cost" slightly differently on purpose — Overheads.js uses pure purchase spend (no opening/closing adjustment), Monthly/Annual Summary, Period Comparison and Budget vs Actual use the full COGS formula above, and these can legitimately show different percentages for the same month. This is the single most common source of "why don\'t these two pages match" support questions — see the callout on Overheads and Monthly Summary below. What is NOT a legitimate difference: all four of those pages now net the bill-level discount off purchases the same way, so their Net Purchases and COGS for one month agree to the rupee. Until S720 three of them did not, which made every genuine difference above harder to explain.',
  ...
  // ───────────────────────────── Operations ─────────────────────────────
  {
    ...
    sections: [
      ...
      },
      {
        ...
        workflow: [
          'Client view: a table of the client\'s own periods, newest first, with a + New Period button (pick BS Year + BS Month) and a Close & Start Next / Reopen action per row. Since S738 a period created by hand carries the previous EXISTING period\'s closing count into its opening stock, exactly as Close & Start Next does — and the notice says which month it came from, or that there was nothing to carry.',
        ...
        fields: [
          ...
          { label: 'Post POS bills to Inventory', desc: 'Posts POS bills that closed while no Inventory period existed for their date. Admin sees it on every row; everyone else only on the OPEN period, because it writes sales_entries and stock_movements and a closed month is read-only for the client\'s logins on every other page too. Posting into a closed month (admin) leaves its frozen Monthly Report stale — the notice says so and names Regenerate Snapshot.' },
      ...
      },
      {
        ...
        fields: [
          ...
          { label: 'Unit Price on the PO', desc: 'A pre-agreed price carried straight through to the resulting purchase entry — it does not re-check against the item\'s live Item Master rate at receive time. It is per BASE unit, so no conversion factor is applied anywhere on this page (unlike a Purchase Bill, whose qty is in purchase units).' },
      ...
      },
      {
        ...
        workflow: [
          ...
          'A failed read renders the "Could not load" card and nothing else — no tabs, no Save All. Before S695 the reads dropped their errors, every cell rendered blank, and Save All then DELETED the server\'s real rows for every visible item, since a blank means "no row".',
    ...
    sections: [
      {
        ...
        formulas: [
          ...
          'Status: No activity if the item has no opening, purchases, usage, wastage, staff meals or closing row this period; else Out if on-hand ≤0; Low if par>0 and on-hand ≤ par; else OK. "Items Tracked" and "Out of Stock" exclude the no-activity items.',
        ],
        gotchas: [
    ...
    sections: [
      {
        ...
        gotchas: [
          ...
          'The write scope is the union over the tabs the VIEWER has (TAB_FIELDS), not a fixed list, and that is what keeps the threshold validation self-limiting: an admin has no Thresholds tab, so a client row left holding a 0 or a critical level below its warning level — both saveable before S730 — cannot refuse the admin\'s branding or consultant save. It used to, and then sent them to setActiveTab(\'Thresholds\'): a tab absent from ADMIN_TABS, so no tab rendered as selected and the panel appeared with nothing above it.',
          'On the admin side, Invoice Prefix and VAT Registered both reach bills ALREADY ISSUED. `pos_orders` stores only invoice_no and invoice_fy; the printed number and the TI/PB bill type are assembled when a bill is printed, from the settings row current at that moment. So changing the prefix re-numbers every past invoice and switching VAT off reprints past tax invoices as PAN bills. Both now ask first and name the consequence.',
