    ...
    sections: [
      ...
      },
      {
        ...
        fields: [
          ...
          { label: 'Purchase Qty / Rate', desc: 'An entry aid, not what is stored. Type the pack you bought — "1000 GM for NPR 500" — and the item is saved as the price of ONE base unit (NPR 0.50 per GM). Every item lives in its smallest form, so Rate on the list IS the per-unit cost that recipe costing and stock valuation read. Leave Purchase Qty at 1 when Rate is already per unit.' },
          ...
          { label: '"Price per unit (NPR)" field', desc: 'The other way in: type the cost of ONE base unit and Rate fills itself as per-unit × Purchase Qty. Either box gets you to the same saved row. Changing Purchase Qty afterwards re-scales Rate from this entered per-unit price rather than leaving a stale pack rate behind. (This replaced an older "Total (NPR)" box that divided instead of multiplying — any item entered through that box carried a per-unit rate Purchase-Qty× too small, mispricing it in every valuation at once.)' },
        ...
        formulas: [
          'Saved rate = entered Rate ÷ entered Purchase Qty — the price of one base unit.',
        ...
        gotchas: [
          'The Rate box while you type is the price of the whole Purchase Qty pack; the rate that gets SAVED is per one base unit. A 1 KG bag tracked as 1000 GM and costing NPR 500 is entered as Purchase Qty 1000, Rate 500 and stored as 0.50 per GM. The green line under the form always states what will be saved. A sub-paisa rate is legitimate (a PCS item bought by the 1000) and displays with up to 6 decimals rather than a misleading flat 0.00.',
          'Purchase Qty is no longer locked to the conversion factor. Buying in CTN and counting in BTL is described by the Conversion tab alone — that is what the Purchase Bill reads to decide whether its Qty column means cartons or bottles.',
