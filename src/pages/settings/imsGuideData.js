    ...
    sections: [
      {
        ...
        fields: [
          ...
          { label: 'Weather (Owner, Growth+, S784)', desc: 'weather_city / weather_lat / weather_lon (written together by a city picker — the app\'s Permissions-Policy blocks device geolocation) and rain_sales_pct. Only the Owner sees the tab (an IMS manager does not), and it saves only these four columns. The coordinates are what the weather-forecast Edge Function reads, from the row and never from the request. rain_sales_pct is refused outside 30–150 and at 0; blank is stored as NULL and means no adjustment. rain_sales_pct is consumed only by the Dashboard\'s Daily Purchases vs Sales sales forecast; the city also drives the Dashboard header\'s weather strip (S785), for every client with the flag, whichever modules it runs.' },
        ],
        formulas: [],
        gotchas: [
