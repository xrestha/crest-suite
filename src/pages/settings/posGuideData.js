    ...
    sections: [
      {
        ...
        workflow: [
          ...
          'On a phone (under 700px wide) the menu takes the full width and the order is a bottom sheet: the TOTAL, Send/Update, Payment and KOT/BOT always show, and "▴ Order" opens the lines (S776). Before, the fixed 320px cart left a 70px menu column and taps on a tile landed on the cart. On any touch screen the order screen\'s controls are 44px or larger; a desktop keeps its density. Send and save messages show directly above the Send button, not in the top bar.',
          ...
          'A close button never looks ready while something is missing (S776). Where a discount reason, the buyer\'s name and phone, a comp reason or a void reason is still needed, the button says so in place of "Confirm Payment", and pressing it shows the reason and moves to that box, opening Items or Buyer details first. Void asks for confirmation, naming the reason and any lines the kitchen already has. The order screen\'s Payment button and a Credit Note\'s Issue & Print behave the same way.',
        ...
        fields: [
          ...
          { label: 'A close on a bad connection', desc: 'Every step of closing a bill has a time limit (about 20 seconds), and the Confirm button names the step it is on. The order since S776: the Split payment lines, then the PRINT, then the table, booking, Inventory posting, customer book and loyalty — so the guest gets the bill before the bookkeeping, and a failure after the print is a warning on the floor, never a refusal. If the close write goes out and its answer is lost, the till reads the bill back: closed by this login means it finishes and prints; still open means press again; unreadable means it says it does not know. The next press, or Cancel, checks again before doing anything, so a bill that did close is never blamed on "another till" and never charged twice. The floor then names the bill number, the amount and the change.' },
      ...
      },
      {
        ...
        workflow: [
          ...
          'The guest\'s screen tracks THEIR order through five labelled steps — Sent → Accepted → In kitchen → Cooking → Ready — driven by the tickets on the bill that took it (get_guest_order_progress, S767), including the prep-time countdown the kitchen entered on the KDS. It never moves backwards, ends with "Your bill is closed. Thank you!" when that bill closes, and a second order from the same table is tracked on its own.',
          'On a long menu (more than 30 dishes) a search box sits above the section buttons; a dish with no photo or description is one line (name, price, Add), so a big menu is a few screens, not dozens. Sections follow the Recipe Categories order, which the Owner or a manager sets from POS Setup → Guest Menu or Settings → Recipe Categories.',
        ],
        fields: [
        ...
        gotchas: [
          ...
          'The tracker used to follow the TABLE\'s kitchen status, so a second-round order nobody had accepted showed the first round\'s "Ready", and a paid bill fell back to "heading to the kitchen" with a chime. Since S767 staff accepting a guest order records which bill it went onto (pos_guest_order_requests.order_id), and the tracker reads only that bill\'s tickets sent after the order was placed.',
          'The restaurant name and logo on the guest menu are the Owner\'s alone — the database refuses a manager or waiter changing them (S767). The public booking page shows the same name and logo (get_booking_page returns them since migration 20260921110000). Dish, choice and category names typed in ALL CAPITALS show in normal capitals on the guest menu only.',
