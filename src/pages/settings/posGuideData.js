  ...
  // ───────────────────────────── Floor ─────────────────────────────
  {
    ...
    sections: [
      ...
      },
      {
        ...
        fields: [
          { label: 'The unstarted alert (S763)', desc: 'A new ticket chimes as it lands — three rising notes, twice, loud enough to carry across a kitchen. If one then sits in New past 8 minutes with nobody pressing Start, a bar drops down over the board naming the oldest one and the chime REPEATS every 20 seconds until someone takes it, escalating to red and a harder tone past 15 minutes. Those are the same two marks the card\'s △ and ▲ already use, so the bar and the board can never disagree. Mute 5 min silences the sound and leaves the bar up, because the ticket is still unstarted. It deliberately does not fire for every unstarted ticket: a kitchen working a queue always has several, and an alarm going off all service is one that gets muted on the first night and never unmuted.' },
