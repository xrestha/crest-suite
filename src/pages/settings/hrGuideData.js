    ...
    sections: [
      {
        ...
        workflow: [
          ...
          'Reopen (HR manager and above, S620) reverses exactly what Finalize wrote: deletes its advance-repayment rows (reactivating anything with balance again), and puts the TADA claims IT marked Paid (Payroll) back to Approved — never one a manager settled by hand.',
        ...
        formulas: [
          ...
          'On Finalize: payslips lock; each payslip\'s advance deduction is recorded as repayment rows against that employee\'s advances that are due this month, oldest first — idempotent, so a Reopen + re-Finalize never doubles them; an advance reaching zero is settled (a database trigger keeps status in step with the balance); the TADA claims on the payslips are marked Paid (Payroll).',
        ],
        gotchas: [
    ...
    sections: [
      {
        ...
        fields: [
          { label: 'TDS Certificate', desc: 'Per employee, per fiscal year — YTD withholding evidence. Lists festival allowances and incentives in their own table (by their Paid in month) and includes them in total income and total tax withheld. The company PAN in its header comes from the VAT/PAN number in Settings (Nepal uses one number for both); it prints a blank line only when genuinely unset.' },
      ...
      },
      {
        ...
        gotchas: [
          ...
          'Finalize does not delete the leaver\'s HR, IMS or POS staff login, and access_blocked stops the next Staff app sign-in without ending one already open.',
        ],
