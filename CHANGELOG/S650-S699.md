...
**Two switches remain deliberately off.** `SITE_ORIGIN` is unset, so `legalReadiness()` still
reports not-ready and `legalAbsoluteUrl()` returns a visibly broken `[SET REACT_APP_SITE_ORIGIN]/...`
rather than a plausible wrong domain — a contract citing the wrong URL is worse than one that admits
it does not know, because only the second gets noticed before signature. And `requiresReacceptance`
is `false`: turning it on blocks every tenant Owner until they accept, which is the point eventually
(no existing client has ever recorded consent) but is a customer-facing event, not a deploy detail.
